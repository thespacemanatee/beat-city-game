...
{
    /// <summary>
    ///     This class lets you bind keys to specific slots in a target inventory, and associate an action to execute when that
    ///     key is pressed.
    ...
    public class InventoryInputActions : MonoBehaviour
    {
        ...
        public Inventory TargetInventory
        {
            get
            {
                ...
                if (_targetInventory == null) _targetInventory = Inventory.FindInventory(TargetInventoryName, PlayerID);
        ...
        {
            if (binding.SlotIndex > _targetInventory.Content.Length) return;
            if (_targetInventory.Content[binding.SlotIndex] == null) return;

            switch (binding.Action)
            {
                case Actions.Equip:
                    MMInventoryEvent.Trigger(MMInventoryEventType.EquipRequest, null, _targetInventory.name,
                        _targetInventory.Content[binding.SlotIndex], 0, binding.SlotIndex, _targetInventory.PlayerID);
                    break;
                case Actions.Use:
                    ...
                    break;
                case Actions.Drop:
                    MMInventoryEvent.Trigger(MMInventoryEventType.Drop, null, _targetInventory.name,
                        _targetInventory.Content[binding.SlotIndex], 0, binding.SlotIndex, _targetInventory.PlayerID);
                    break;
                case Actions.Unequip:
                    MMInventoryEvent.Trigger(MMInventoryEventType.UnEquipRequest, null, _targetInventory.name,
                        _targetInventory.Content[binding.SlotIndex], 0, binding.SlotIndex, _targetInventory.PlayerID);
                    break;
                default:
