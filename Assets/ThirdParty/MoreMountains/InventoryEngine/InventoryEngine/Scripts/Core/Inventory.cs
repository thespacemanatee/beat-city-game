...
namespace MoreMountains.InventoryEngine
{
    ...
    public class Inventory : MonoBehaviour, MMEventListener<MMInventoryEvent>, MMEventListener<MMGameEvent>
    {
        ...
        [Tooltip(
            "This is a realtime view of your Inventory's contents. Don't modify this list via the inspector, it's visible for control purposes only.")]
        public InventoryItem[] Content;

        [Header("Inventory Type")]
        /// whether this inventory is a main inventory or equipment one
        [Tooltip(
            "Here you can define your inventory's type. Main are 'regular' inventories. Equipment inventories will be bound to a certain item class and have dedicated options.")]
        public InventoryTypes InventoryType = InventoryTypes.Main;

        [Header("Target Transform")]
        [Tooltip(
            "The TargetTransform is any transform in your scene at which objects dropped from the inventory will spawn.")]
        /// the transform at which objects will be spawned when dropped
        public Transform TargetTransform;

        [Header("Persistency")]
        [Tooltip(
            "Here you can define whether or not this inventory should respond to Load and Save events. If you don't want to have your inventory saved to disk, set this to false. You can also have it reset on start, to make sure it's always empty at the start of this level.")]
        ...
        {
            if (gameEvent.EventName == "Save" && Persistent) SaveInventory();
            if (gameEvent.EventName == "Load" && Persistent)
            {
                if (ResetThisInventorySaveOnStart) ResetSavedInventory();
                LoadSavedInventory();
            }
        }
        ...
        public virtual void OnMMEvent(MMInventoryEvent inventoryEvent)
        {
            ...
            if (inventoryEvent.TargetInventoryName != name) return;
            ...
            switch (inventoryEvent.InventoryEventType)
            {
                case MMInventoryEventType.Pick:
                    if (inventoryEvent.EventItem.ForceSlotIndex)
                        AddItemAt(inventoryEvent.EventItem, inventoryEvent.Quantity,
                            inventoryEvent.EventItem.TargetIndex);
                    else
        ...
        {
            if (RegisteredInventories == null) RegisteredInventories = new List<Inventory>();
            if (RegisteredInventories.Count > 0)
                for (var i = RegisteredInventories.Count - 1; i >= 0; i--)
        ...
        public virtual bool AddItem(InventoryItem itemToAdd, int quantity)
        {
            ...
            if (NumberOfFilledSlots >= Content.Length) return false;
            while (quantity > 0)
        ...
        {
            if (!InventoryItem.IsNull(Content[destinationIndex])) return false;

            var tempQuantity = quantity;
            if (tempQuantity > itemToAdd.MaximumStack) tempQuantity = itemToAdd.MaximumStack;
        ...
        public virtual bool MoveItem(int startIndex, int endIndex)
        {
            ...
            {
                // we swap our items
                var tempItem = Content[endIndex].Copy();
                Content[endIndex] = Content[startIndex].Copy();
        ...
        public virtual bool RemoveItem(int i, int quantity)
        {
            ...
            {
                var suppressionSuccessful = RemoveItemFromArray(i);
                MMInventoryEvent.Trigger(MMInventoryEventType.ContentChanged, null, name, null, 0, 0, PlayerID);
        ...
        public virtual bool RemoveItemByID(string itemID, int quantity)
        {
            if (quantity < 1)
            {
                Debug.LogWarning("InventoryEngine : you're trying to remove an incorrect quantity (" + quantity +
                                 ") from your inventory.");
                return false;
            }
            ...
            var quantityLeftToRemove = quantity;

            var list = InventoryContains(itemID);
            foreach (var index in list)
            {
                var quantityAtIndex = Content[index].Quantity;
                RemoveItem(index, quantityLeftToRemove);
                quantityLeftToRemove -= quantityAtIndex;
                if (quantityLeftToRemove <= 0) return true;
            }
        ...
        {
            if (NumberOfFreeSlots == 0) return false;
            var i = 0;
            while (i < Content.Length)
            {
                if (InventoryItem.IsNull(Content[i]))
        ...
        public virtual void ResizeArray(int newSize)
        {
            ...
            for (var i = 0; i < Mathf.Min(newSize, Content.Length); i++) temp[i] = Content[i];
            Content = temp;
        }
        ...
        {
            var list = InventoryContains(searchedItemID);
            var total = 0;
            foreach (var i in list) total += Content[i].Quantity;
            return total;
        }
        ...
        public virtual List<int> InventoryContains(ItemClasses searchedClass)
        {
            ...
            for (var i = 0; i < Content.Length; i++)
            {
                if (InventoryItem.IsNull(Content[i])) continue;
                if (Content[i].ItemClass == searchedClass) list.Add(i);
            }
        ...
        {
            var serializedInventory = (SerializedInventory)MMSaveLoadManager.Load(typeof(SerializedInventory),
        ...
        protected virtual void ExtractSerializedInventory(SerializedInventory serializedInventory)
        {
            ...
            for (var i = 0; i < serializedInventory.ContentType.Length; i++)
                if (serializedInventory.ContentType[i] != null && serializedInventory.ContentType[i] != "")
                {
                    _loadedInventoryItem =
                        Resources.Load<InventoryItem>(_resourceItemPath + serializedInventory.ContentType[i]);
                    if (_loadedInventoryItem == null)
                    {
                        Debug.LogError("InventoryEngine : Couldn't find any inventory item to load at " +
                                       _resourceItemPath
                                       + " named " + serializedInventory.ContentType[i] +
                                       ". Make sure all your items definitions names (the name of the InventoryItem scriptable " +
        ...
        /// <summary>
        ///     Triggers the use and potential consumption of the item passed in parameter. You can also specify the item's slot
        ///     (optional) and index.
        /// </summary>
        ...
        public virtual bool UseItem(InventoryItem item, int index, InventorySlot slot = null)
        {
            ...
            {
                MMInventoryEvent.Trigger(MMInventoryEventType.Error, slot, name, null, 0, index, PlayerID);
                return false;
            }
            ...
            if (item.Use(PlayerID))
            {
                ...
                MMInventoryEvent.Trigger(MMInventoryEventType.ItemUsed, slot, name, item.Copy(), 0, index, PlayerID);
                if (item.Consumable) RemoveItem(index, item.ConsumeQuantity);
            }
        ...
        /// <summary>
        ///     Triggers the use of an item, as specified by its name. Prefer this signature over the previous one if you don't
        ///     particularly care what slot the item will be taken from in case of duplicates.
        /// </summary>
        /// <param name="itemName"></param>
        /// <returns></returns>
        public virtual bool UseItem(string itemName)
        {
            var list = InventoryContains(itemName);
            if (list.Count > 0)
            {
                UseItem(Content[list[list.Count - 1]], list[list.Count - 1]);
                return true;
            }
        ...
        public virtual void EquipItem(InventoryItem item, int index, InventorySlot slot = null)
        {
            if (InventoryType == InventoryTypes.Main)
            {
                ...
                {
                    Debug.LogWarning("InventoryEngine Warning : " + Content[index].ItemName +
                                     "'s target equipment inventory couldn't be found.");
                    return;
                }
                ...
                // remove 1 from quantity
                if (item.MoveWhenEquipped) RemoveItem(index, item.Quantity);
                if (oldItem != null)
                {
                ...
                MMInventoryEvent.Trigger(MMInventoryEventType.ItemEquipped, slot, name, item, item.Quantity, index,
                    PlayerID);
            }
        }
        ...
        public virtual void DropItem(InventoryItem item, int index, InventorySlot slot = null)
        {
            ...
            {
                if (item.UnEquip(PlayerID)) DestroyItem(index);
            }
            else
            {
                DestroyItem(index);
        ...
        public virtual void UnEquipItem(InventoryItem item, int index, InventorySlot slot = null)
        {
            ...
            if (!item.UnEquip(PlayerID)) return;
            MMInventoryEvent.Trigger(MMInventoryEventType.ItemUnEquipped, slot, name, item, item.Quantity, index,
                PlayerID);
            ...
            if (item.TargetInventory(PlayerID) != null)
            {
                ...
                if (item.TargetInventory(PlayerID).AddItem(item, item.Quantity))
                    ...
                    MMInventoryEvent.Trigger(MMInventoryEventType.Drop, slot, name, item, item.Quantity, index,
                        PlayerID);
            }
        }
    }
