...
namespace MoreMountains.InventoryEngine
{
    ...
    {
        public enum ManageButtonsModes
        {
            Interactable,
            SetActive
        }
        ...
        [Header("Targets")]
        [MMInformation(
            "Bind here your inventory container (the CanvasGroup that you want to turn on/off when opening/closing the inventory), your main InventoryDisplay, and the overlay that will be displayed under the InventoryDisplay when opened.",
        ...
        [MMInformation(
            "If you set HideContainerOnStart to true, the TargetInventoryContainer defined right above this field will be automatically hidden on Start, even if you've left it visible in Scene view. Useful for setup.",
            MMInformationAttribute.InformationType.Info, false)]
        /// if this is true, the inventory container will be hidden automatically on start
        public bool HideContainerOnStart = true;

        [Header("Permissions")]
        [MMInformation("Here you can decide to have your inventory catch input only when open, or not.",
            MMInformationAttribute.InformationType.Info, false)]
        /// if this is true, the inventory container will be hidden automatically on start
        ...
        [Header("Key Mapping")]
        [MMInformation(
            "Here you need to set the various key bindings you prefer. There are some by default but feel free to change them.",
            MMInformationAttribute.InformationType.Info, false)]
        ...
        [MMCondition("ManageButtons", true)] public ManageButtonsModes ManageButtonsMode = ManageButtonsModes.SetActive;

        /// the button used to equip or use an item
        [MMCondition("ManageButtons", true)] public Button EquipUseButton;

        /// the button used to move an item
        [MMCondition("ManageButtons", true)] public Button MoveButton;

        /// the button used to drop an item
        [MMCondition("ManageButtons", true)] public Button DropButton;

        /// the button used to equip an item
        [MMCondition("ManageButtons", true)] public Button EquipButton;

        /// the button used to use an item
        [MMCondition("ManageButtons", true)] public Button UseButton;

        /// the button used to unequip an item
        [MMCondition("ManageButtons", true)] public Button UnEquipButton;
        ...
        protected virtual void Start()
        {
            ...
            foreach (var go in FindObjectsOfType(typeof(InventoryHotbar)) as InventoryHotbar[])
        ...
        {
            if (inventoryEvent.PlayerID != TargetInventoryDisplay.PlayerID) return;

            if (inventoryEvent.InventoryEventType == MMInventoryEventType.InventoryCloseRequest) CloseInventory();
        }
        ...
        protected virtual void CheckCurrentlySelectedSlot()
        {
            ...
            if (_currentInventorySlot != null) CurrentlySelectedInventorySlot = _currentInventorySlot;
        }

        /// <summary>
        ///     Will turn inventory controls interactable or not based on the currently selected slot, if ManageButtons is set to
        ///     true
        /// </summary>
        protected virtual void HandleButtons()
        {
            ...
            {
                if (_isUseButtonNotNull) SetButtonState(UseButton, CurrentlySelectedInventorySlot.Usable());

                if (_isEquipButtonNotNull) SetButtonState(EquipButton, CurrentlySelectedInventorySlot.Equippable());
                ...
                if (_isMoveButtonNotNull) SetButtonState(MoveButton, CurrentlySelectedInventorySlot.Movable());

                if (_isDropButtonNotNull) SetButtonState(DropButton, CurrentlySelectedInventorySlot.Droppable());
            }
            else
            {
        ...
        public virtual void OpenInventory()
        {
            if (CloseList.Count > 0)
                foreach (var playerID in CloseList)
                    MMInventoryEvent.Trigger(MMInventoryEventType.InventoryCloseRequest, null, "", null, 0, 0,
                        playerID);
            ...
            MMInventoryEvent.Trigger(MMInventoryEventType.InventoryOpens, null,
                TargetInventoryDisplay.TargetInventoryName, TargetInventoryDisplay.TargetInventory.Content[0], 0, 0,
        ...
        public virtual void CloseInventory()
        {
            ...
            MMInventoryEvent.Trigger(MMInventoryEventType.InventoryCloses, null,
                TargetInventoryDisplay.TargetInventoryName, null, 0, 0, TargetInventoryDisplay.PlayerID);
        ...
        protected virtual void HandleInventoryInput()
        {
            ...
            // equip or use
            if (Input.GetKeyDown(EquipOrUseKey) || Input.GetKeyDown(EquipOrUseAltKey)) EquipOrUse();
        ...
        {
            if (CurrentlySelectedInventorySlot.Equippable()) CurrentlySelectedInventorySlot.Equip();
            if (CurrentlySelectedInventorySlot.Usable()) CurrentlySelectedInventorySlot.Use();
        }
