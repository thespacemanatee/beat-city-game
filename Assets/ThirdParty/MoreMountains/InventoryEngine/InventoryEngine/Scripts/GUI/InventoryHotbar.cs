...
namespace MoreMountains.InventoryEngine
{
	...
	public class InventoryHotbar : InventoryDisplay
    {
        ...
        {
            for (var i = TargetInventory.Content.Length - 1; i >= 0; i--)
                if (!InventoryItem.IsNull(TargetInventory.Content[i]))
                {
                    if (ActionOnKey == HotbarPossibleAction.Equip && SlotContainer[i] != null) SlotContainer[i].Equip();
                    if (ActionOnKey == HotbarPossibleAction.Use && SlotContainer[i] != null) SlotContainer[i].Use();
                    return;
                }
        }
    }
