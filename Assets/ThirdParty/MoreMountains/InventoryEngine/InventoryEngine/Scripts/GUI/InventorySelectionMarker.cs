...
namespace MoreMountains.InventoryEngine
{
    ...
    {
        [MMInformation(
            "The selection marker will highlight the current selection. Here you can define its transition speed and minimal distance threshold (it's usually ok to leave it to default).",
        ...
        private void Update()
        {
            ...
            if (_currentSelection.gameObject.MMGetComponentNoAlloc<InventorySlot>() == null) return;
            ...
            if (Vector3.Distance(transform.position, _currentSelection.transform.position) > MinimalTransitionDistance)
            {
                ...
                _rectTransform.localScale = Vector3.Lerp(_originLocalScale,
                    _currentSelection.GetComponent<RectTransform>().localScale, _deltaTime);
                _rectTransform.sizeDelta = Vector3.Lerp(_originSizeDelta,
                    _currentSelection.GetComponent<RectTransform>().sizeDelta, _deltaTime);
            }
            else
            {
                _originIsNull = true;
