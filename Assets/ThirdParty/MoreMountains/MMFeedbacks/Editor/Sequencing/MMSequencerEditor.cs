    ...
    public class MMSequencerEditor : Editor
    {
        ...
        protected virtual void FillControlList()
        {
            ...
            if (_targetSequencer.Sequence != null)
            {
                ...
                foreach (var track in _targetSequencer.Sequence.SequenceTracks) _trackControlLastUseTimestamps.Add(0f);
            }
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.Space();
            DrawSequenceIndexes();
            for (var i = 0; i < _targetSequencer.Sequence.SequenceTracks.Count; i++) DrawTrack(i);
        ...
        protected virtual void DrawSequenceIndexes()
        {
            ...
            var label = "";
            for (var i = 0; i < _targetSequencer.SequencerLength; i++)
            {
                label = i < 10 ? " " + i : i.ToString();
        ...
        protected virtual void DrawTrack(int trackIndex)
        {
            ...
            if (_targetSequencer.Sequence.QuantizedSequence.Count !=
                _targetSequencer.Sequence.SequenceTracks.Count) return;
            ...
            for (var i = 0; i < _targetSequencer.Sequence.QuantizedSequence[trackIndex].Line.Count; i++)
            {
                // handle new lines
        ...
        protected virtual void DrawSequenceButton(int trackIndex, int sequenceIndex, Color buttonColor)
        {
            ...
            {
                var active = _targetSequencer.Sequence.QuantizedSequence[trackIndex].Line[sequenceIndex].ID ==
        ...
        {
            var x = Time.time - lastBeatTimestamp;
            var A = 0f;
            var B = 60f / _targetSequencer.BPM / 4f;
            var C = 0f;
            var D = 1f;
            var t = C + (x - A) / (B - A) * (D - C);
