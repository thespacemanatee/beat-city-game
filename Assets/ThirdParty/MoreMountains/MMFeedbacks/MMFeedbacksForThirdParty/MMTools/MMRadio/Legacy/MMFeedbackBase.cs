...
namespace MoreMountains.Feedbacks
{
    ...
    public abstract class MMFeedbackBase : MMFeedback
    {
        ...
        protected virtual void Instant()
        {
            ...
            foreach (var target in _targets) target.Target.SetLevel(target.InstantLevel);
        }
        ...
        protected virtual void SetValues(float time, float feedbacksIntensity)
        {
            ...
            foreach (var target in _targets)
            {
                var intensity = MMTween.Tween(time, 0f, 1f, target.RemapLevelZero, target.RemapLevelOne,
                    target.LevelCurve);
