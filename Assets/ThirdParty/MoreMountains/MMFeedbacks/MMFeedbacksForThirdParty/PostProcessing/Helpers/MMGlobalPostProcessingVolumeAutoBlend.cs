...
namespace MoreMountains.FeedbacksForThirdParty
{
    ...
    public class MMGlobalPostProcessingVolumeAutoBlend : MonoBehaviour
    {
        ...
        protected virtual void Update()
        {
            ...
            var timeElapsed = GetTime() - _startTime;
            if (timeElapsed < BlendDuration)
            {
                var remapped = MMFeedbacksHelpers.Remap(timeElapsed, 0f, BlendDuration, 0f, 1f);
            ...
            else
            {
                ...
                _blending = false;
                if (DisableVolumeOnZeroWeight && _volume.weight == 0f) _volume.enabled = false;
                if (DisableSelfAfterEnd) enabled = false;
            }
        }
        ...
        {
            if (BlendTriggerMode == BlendTriggerModes.OnEnable && !_blending) Blend();
        }
        ...
        {
            if (_blending && !Interruptable) return;
        ...
        protected virtual void StartBlending()
        {
            ...
            _blending = true;
            enabled = true;
            if (DisableVolumeOnZeroWeight) _volume.enabled = true;
        }
