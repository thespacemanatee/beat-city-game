...
namespace MoreMountains.Feedbacks
{
    ...
    public class MMF_Player : MMFeedbacks
    {
        ...
        public override float TotalDuration
        {
            ...
            {
                var total = 0f;
                if (FeedbacksList == null) return InitialDelay;
        ...
        protected override void Awake()
        {
            ...
            if (AutoPlayOnEnable)
            {
                var playerEnabler = GetComponent<MMF_PlayerEnabler>();
                if (playerEnabler == null) playerEnabler = gameObject.AddComponent<MMF_PlayerEnabler>();
                playerEnabler.TargetMmfPlayer = this;
            }

            if (InitializationMode == InitializationModes.Awake && Application.isPlaying) Initialization();
            CheckForLoops();
        }
        ...
        {
            if (InitializationMode == InitializationModes.Start && Application.isPlaying) Initialization();
        ...
        {
            if (FeedbacksList == null) FeedbacksList = new List<MMF_Feedback>();
        }
        ...
        protected override void OnEnable()
        {
            ...
            foreach (var feedback in FeedbacksList) feedback.CacheRequiresSetup();
        }

        /// <summary>
        ///     A public method to initialize the feedback, specifying an owner that will be used as the reference for position and
        ///     hierarchy by feedbacks
        /// </summary>
        ...
        /// <summary>
        ///     Plays all feedbacks, specifying a position and intensity. The position may be used by each Feedback and taken into
        ///     account to spark a particle or play a sound for example.
        ///     The feedbacks intensity is a factor that can be used by each Feedback to lower its intensity, usually you'll want
        ///     to define that attenuation based on time or distance (using a lower
        ...
        /// <summary>
        ///     Plays all feedbacks using the MMFeedbacks' position as reference, and no attenuation, and in reverse (from bottom
        ///     to top)
        /// </summary>
        ...
        public override void PlayFeedbacksOnlyIfNormalDirection(Vector3 position, float feedbacksIntensity = 1.0f,
            bool forceRevert = false)
        {
            if (Direction == Directions.TopToBottom) PlayFeedbacks(position, feedbacksIntensity, forceRevert);
        }

        /// <summary>
        ///     A public coroutine you can call externally when you want to yield in a coroutine of yours until the MMFeedbacks has
        ///     stopped playing
        ...
        protected override void PlayFeedbacksInternal(Vector3 position, float feedbacksIntensity,
            bool forceRevert = false)
        {
            if (IsPlaying && !CanPlayWhileAlreadyPlaying) return;

            if (!EvaluateChance()) return;
            ...
            if (!GlobalMMFeedbacksActive) return;
            ...
            for (var i = 0; i < count; i++)
                if (FeedbacksList[i] != null)
                {
                    if (FeedbacksList[i].Pause != null && FeedbacksList[i].Active &&
                        FeedbacksList[i].ShouldPlayInThisSequenceDirection) _pauseFound = true;
                    if (FeedbacksList[i].HoldingPause && FeedbacksList[i].Active &&
                        FeedbacksList[i].ShouldPlayInThisSequenceDirection) _pauseFound = true;
                }
        ...
        protected override void Update()
        {
            if (_shouldStop)
            {
                if (HasFeedbackStillPlaying()) return;
                IsPlaying = false;
        ...
        protected override IEnumerator PausedFeedbacksCo(Vector3 position, float feedbacksIntensity)
        {
            ...
            var i = Direction == Directions.TopToBottom ? 0 : FeedbacksList.Count - 1;
            ...
            while (i >= 0 && i < count)
            {
                ...
                if (FeedbacksList[i] == null) yield break;

                if ((FeedbacksList[i].Active && FeedbacksList[i].ScriptDrivenPause) || InScriptDrivenPause)
                {
                    InScriptDrivenPause = true;

                    var inAutoResume = FeedbacksList[i].ScriptDrivenPauseAutoResume > 0f;
                    var scriptDrivenPauseStartedAt = Time.unscaledTime;
                    var autoResumeDuration = FeedbacksList[i].ScriptDrivenPauseAutoResume;
                ...
                if (FeedbacksList[i].Active
                    && (FeedbacksList[i].HoldingPause || FeedbacksList[i].LooperPause)
                ...
                // plays the feedback
                if (FeedbackCanPlay(FeedbacksList[i])) FeedbacksList[i].Play(position, feedbacksIntensity);
                    ...
                    FeedbacksList[i].ShouldPlayInThisSequenceDirection)
                {
                    ...
                    if (FeedbacksList[i].Chance < 100)
                    {
                        ...
                        if (random > FeedbacksList[i].Chance) shouldPause = false;
                    }
                ...
                if (FeedbacksList[i].Active)
                    ...
                    {
                        var feedbackDuration = FeedbacksList[i].TotalDuration;
                        ...
                        (FeedbacksList[i] as MMF_Looper).InInfiniteLoop))
                {
                    ...
                    var loopAtLastPause = (FeedbacksList[i] as MMF_Looper).LoopAtLastPause;
                    var loopAtLastLoopStart = (FeedbacksList[i] as MMF_Looper).LoopAtLastLoopStart;

                    var newi = 0;

                    var j = Direction == Directions.TopToBottom ? i - 1 : i + 1;

                    var listCount = FeedbacksList.Count;
                    while (j >= 0 && j <= listCount)
                    {
                        ...
                        if (FeedbacksList[j].LooperStart
                            && loopAtLastLoopStart
            ...
            var unscaledTimeAtEnd = Time.unscaledTime;
            while (Time.unscaledTime - unscaledTimeAtEnd < _holdingMax) yield return null;
            while (HasFeedbackStillPlaying()) yield return null;
            IsPlaying = false;
            Events.TriggerOnComplete(this);
            ApplyAutoRevert();
        }
            ...
            bool stopAllFeedbacks = true)
        {
            ...
            {
                var count = FeedbacksList.Count;
                for (var i = 0; i < count; i++) FeedbacksList[i].Stop(position, feedbacksIntensity);
            }
        ...
        /// <summary>
        ///     Calls each feedback's Reset method if they've defined one. An example of that can be resetting the initial color of
        ///     a flickering renderer.
        /// </summary>
        ...
        {
            if (FeedbacksList.Count < id) return;
        ...
        /// <summary>
        ///     This will return true if the conditions defined in the specified feedback's Timing section allow it to play in the
        ///     current play direction of this MMFeedbacks
        /// </summary>
        ...
        protected bool FeedbackCanPlay(MMF_Feedback feedback)
        {
            if (feedback.Timing.MMFeedbacksDirectionCondition == MMFeedbackTiming.MMFeedbacksDirectionConditions.Always)
                return true;
            if ((Direction == Directions.TopToBottom && feedback.Timing.MMFeedbacksDirectionCondition ==
        ...
        public virtual void RefreshCache()
        {
            ...
            for (var i = FeedbacksList.Count - 1; i >= 0; i--)
                if (FeedbacksList[i] == null)
                {
        ...
        protected override void OnDestroy()
        {
            ...
            foreach (var feedback in FeedbacksList) feedback.OnDestroy();
        }
