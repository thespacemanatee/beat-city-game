...
{
    /// <summary>
    ///     This feedback will let you modify the scale of an object on an axis while the other two axis (or only one) get
    ///     automatically modified to conserve mass
    /// </summary>
    [AddComponentMenu("")]
    ...
    public class MMF_SquashAndStretch : MMF_Feedback
    {
        ...
        protected override void CustomInitialization(MMF_Player owner)
        {
            ...
            if (Active && SquashAndStretchTarget != null) GetInitialScale();
        }
        ...
        {
            if (!Active || !FeedbackTypeAuthorized || SquashAndStretchTarget == null) return;
        ...
        {
            if (SquashAndStretchTarget == null) yield break;
            ...
            while (journey >= 0 && journey <= FeedbackDuration && FeedbackDuration > 0)
            {
                ...
                var newScale = Mathf.LerpUnclamped(_initialAxisScale, DestinationScale,
            ...
            PossibleAxis axis, float remapCurveZero = 0f, float remapCurveOne = 1f)
        {
            ...
            while (journey >= 0 && journey <= duration && duration > 0)
            {
                ...
                var newScale = curve.Evaluate(percent) + Offset;
                newScale = MMFeedbacksHelpers.Remap(newScale, 0f, 1f, remapCurveZero, remapCurveOne);
                if (Mode == Modes.Additive) newScale += _initialAxisScale;
            ...
            if (Mode == Modes.Additive) finalScale += _initialAxisScale;
        ...
        {
            var invertScale = 1 / Mathf.Sqrt(newScale);
            switch (Axis)
            {
                case PossibleAxis.XtoYZ:
