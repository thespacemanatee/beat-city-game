...
{
    /// <summary>
    ///     A feedback used to trigger an animation (bool, int, float or trigger) on the associated animator, with or without
    ///     randomness
    /// </summary>
    [AddComponentMenu("")]
    [FeedbackHelp(
    ...
    public class MMF_Animation : MMF_Feedback
    {
        ...
        protected override void CustomInitialization(MMF_Player owner)
        {
            ...
            foreach (var name in RandomTriggerParameterNames) _randomTriggerParameters.Add(Animator.StringToHash(name));

            _randomBoolParameters = new List<int>();
            foreach (var name in RandomBoolParameterNames) _randomBoolParameters.Add(Animator.StringToHash(name));
        }
        ...
        protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
        {
            ...
            {
                var randomParameter = _randomTriggerParameters[Random.Range(0, _randomTriggerParameters.Count)];
            ...
            if (UpdateRandomBool)
            {
                var randomParameter = _randomBoolParameters[Random.Range(0, _randomBoolParameters.Count)];
            ...
            switch (IntValueMode)
            {
                ...
                case ValueModes.Incremental:
                    var newValue = BoundAnimator.GetInteger(_intParameter) + IntIncrement;
                    BoundAnimator.SetInteger(_intParameter, newValue);
                    break;
                case ValueModes.Random:
                    var randomValue = Random.Range(IntValueMin, IntValueMax);
            ...
            switch (FloatValueMode)
            {
                case ValueModes.Constant:
                    ...
                    break;
                case ValueModes.Incremental:
                    var newValue = BoundAnimator.GetFloat(_floatParameter) + FloatIncrement * intensityMultiplier;
                    BoundAnimator.SetFloat(_floatParameter, newValue);
                    break;
                case ValueModes.Random:
                    var randomValue = Random.Range(FloatValueMin, FloatValueMax) * intensityMultiplier;
        ...
        {
            if (!Active || !UpdateBool || !FeedbackTypeAuthorized) return;
