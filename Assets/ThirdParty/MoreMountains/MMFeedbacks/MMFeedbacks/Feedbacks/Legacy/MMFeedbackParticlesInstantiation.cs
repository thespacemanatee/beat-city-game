...
namespace MoreMountains.Feedbacks
{
    ...
    public class MMFeedbackParticlesInstantiation : MMFeedback
    {
        ...
        protected virtual void InstantiateParticleSystem()
        {
            ...
            if (NestParticles)
            {
                if (PositionMode == PositionModes.FeedbackPosition) newParent = transform;
            ...
            if (RandomParticlePrefabs.Count > 0)
            {
                if (Mode == Modes.Cached)
                {
                    ...
                    {
                        var newSystem = Instantiate(system, newParent);
                        if (newParent == null)
                ...
                else
                {
                    ...
                    _instantiatedParticleSystem = Instantiate(RandomParticlePrefabs[random], newParent);
                    if (newParent == null)
            ...
            {
                _instantiatedParticleSystem = Instantiate(ParticlesPrefab, newParent);
                if (newParent == null)
                    SceneManager.MoveGameObjectToScene(_instantiatedParticleSystem.gameObject, gameObject.scene);
            }
        ...
        protected virtual void PositionParticleSystem(ParticleSystem system)
        {
            ...
            if (ApplyRotation) system.transform.rotation = GetRotation(transform);

            if (ApplyScale) system.transform.localScale = GetScale(transform);
