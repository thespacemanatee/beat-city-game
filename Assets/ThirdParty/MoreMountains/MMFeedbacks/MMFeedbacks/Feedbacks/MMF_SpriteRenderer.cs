...
{
    /// <summary>
    ///     This feedback will let you change the color of a target sprite renderer over time, and flip it on X or Y. You can
    ///     also use it to command one or many MMSpriteRendererShakers.
    /// </summary>
    [AddComponentMenu("")]
    [FeedbackHelp(
    ...
    public class MMF_SpriteRenderer : MMF_Feedback
    {
        ...
        protected virtual IEnumerator SpriteRendererToDestinationSequence(bool andBack)
        {
            ...
            while (journey >= 0 && journey <= FeedbackDuration && FeedbackDuration > 0)
            {
                ...
                var evalTime = ToDestinationColorCurve.Evaluate(remappedTime);
