...
namespace MoreMountains.Feedbacks
{
    ...
    public class MMF_ParticlesInstantiation : MMF_Feedback
    {
        ...
        protected virtual void InstantiateParticleSystem()
        {
            ...
            if (NestParticles)
            {
                if (PositionMode == PositionModes.FeedbackPosition) newParent = Owner.transform;
                if (PositionMode == PositionModes.Transform) newParent = InstantiateParticlesPosition;
            }
            ...
            if (RandomParticlePrefabs.Count > 0)
            {
                if (Mode == Modes.Cached)
                {
                    ...
                    foreach (var system in RandomParticlePrefabs)
                    {
                        var newSystem = Object.Instantiate(system, newParent);
                        if (newParent == null)
                ...
                {
                    var random = Random.Range(0, RandomParticlePrefabs.Count);
                    _instantiatedParticleSystem = Object.Instantiate(RandomParticlePrefabs[random], newParent);
                    if (newParent == null)
            ...
            {
                _instantiatedParticleSystem = Object.Instantiate(ParticlesPrefab, newParent);
                if (newParent == null)
            ...
            if (_instantiatedRandomParticleSystems != null && _instantiatedRandomParticleSystems.Count > 0)
                foreach (var system in _instantiatedRandomParticleSystems)
        ...
        protected virtual void PositionParticleSystem(ParticleSystem system)
        {
            ...
            if (ApplyRotation) system.transform.rotation = GetRotation(Owner.transform);

            if (ApplyScale) system.transform.localScale = GetScale(Owner.transform);
        ...
        protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
        {
            ...
            if (_instantiatedParticleSystem != null)
            {
                if (ForceSetActiveOnPlay) _instantiatedParticleSystem.gameObject.SetActive(true);
            ...
            if (_instantiatedRandomParticleSystems != null && _instantiatedRandomParticleSystems.Count > 0)
            {
                foreach (var system in _instantiatedRandomParticleSystems)
                {
                    if (ForceSetActiveOnPlay) system.gameObject.SetActive(true);
                    system.Stop();
                    system.transform.position = GetPosition(position);
                }

                var random = Random.Range(0, _instantiatedRandomParticleSystems.Count);
