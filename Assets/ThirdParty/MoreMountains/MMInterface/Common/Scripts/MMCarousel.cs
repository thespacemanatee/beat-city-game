...
namespace MoreMountains.MMInterface
{
	...
	public class MMCarousel : MonoBehaviour
    {
        ...
        protected virtual void Initialization()
        {
            ...
            if (InitialFocus != null) EventSystem.current.SetSelectedGameObject(InitialFocus, null);

            if (ForceMouseVisible) Cursor.visible = true;
        }
        ...
        protected virtual void HandleFocus()
        {
            if (!_lerping && Time.timeSinceLevelLoad > 0.5f)
                ...
                {
                    if (UICamera.WorldToScreenPoint(EventSystem.current.currentSelectedGameObject.transform.position)
                            .x < 0) MoveLeft();
                    if (UICamera.WorldToScreenPoint(EventSystem.current.currentSelectedGameObject.transform.position)
                            .x > Screen.width) MoveRight();
                }
        }
        ...
        {
            var timeSinceStarted = Time.time - _lerpStartedTimestamp;
            var percentageComplete = timeSinceStarted / MoveDuration;

            _rectTransform.anchoredPosition = Vector2.Lerp(_startPosition, _targetPosition, percentageComplete);

            //When we've completed the lerp, we set _isLerping to false
            if (percentageComplete >= ThresholdInPercent) _lerping = false;
        }
    }
}