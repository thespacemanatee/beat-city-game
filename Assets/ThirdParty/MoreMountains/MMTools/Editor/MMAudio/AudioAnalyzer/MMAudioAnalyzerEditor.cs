...
namespace MoreMountains.Tools
{
    ...
    public class MMAudioAnalyzerEditor : Editor
    {
        ...
        protected bool _bandValuesFoldout;
        ...
        protected virtual void DrawBandTable()
        {
            ...
            if (_bandValuesFoldout)
            {
                ...
                GUILayout.EndHorizontal();
                for (var i = 0; i < _numberOfBands; i++)
                {
                    GUILayout.BeginHorizontal();
        ...
        protected virtual void DrawBeats()
        {
            ...
            var beatsBoxSquareSize = _beatsBoxHeight - 2 * margin;

            var boxesPerLine =
                (int)Mathf.Round((_inspectorWidth - margin - 3 * _externalMargin) / (beatsBoxSquareSize + margin));
            var numberOfLines = (int)(length / boxesPerLine) + 1;
            var boxHeight = _beatsBoxHeight * numberOfLines - margin * (numberOfLines - 1);
            ...
            for (var i = 0; i < length; i++)
            {
                ...
                var boxX = _boxPosition.x + margin + counter * (beatsBoxSquareSize + margin);
                var boxY = _boxPosition.y + margin + lineCounter * (beatsBoxSquareSize + margin);
                ...
                // draw number
                var labelX = i > 9 ? boxX + beatsBoxSquareSize / 4 - 2 : boxX + beatsBoxSquareSize / 4 + 2;
        ...
        protected virtual void DrawBandVisualization()
        {
            ...
            if (BandPeaks != null && BandPeaks.arraySize == _numberOfBands)
                for (var i = 0; i < _numberOfBands; i++)
                {
                    var peak = BandPeaks.GetArrayElementAtIndex(i).floatValue;
                    if (Active)
                    {
                        if (Time.time - LastPeaksAt.GetArrayElementAtIndex(i).floatValue < _peakShowDuration)
            ...
            for (var i = 0; i < _numberOfBands; i++)
                if (Active)
                {
                    var bandLevel = BufferedBandLevels.GetArrayElementAtIndex(i).floatValue;
            ...
            for (var i = 0; i < _numberOfBands; i++)
            {
                if (Active)
                {
                    var bandLevel = BandLevels.GetArrayElementAtIndex(i).floatValue;
                    _columnHeight = MMMaths.Remap(bandLevel, 0f, 1f, 0f, _maxColumnHeight);
        ...
        protected virtual void DrawBandVisualizationNormalized()
        {
            ...
            for (var i = 0; i < _numberOfBands; i++)
                if (Active)
                {
                    var bandLevel = NormalizedBufferedBandLevels.GetArrayElementAtIndex(i).floatValue;
                    _columnHeight = MMMaths.Remap(bandLevel, 0f, 1f, 0f, _maxColumnHeight);
                    _barColor = Time.time - LastPeaksAt.GetArrayElementAtIndex(i).floatValue < _peakShowDuration / 3f
                        ? _activePeakColor
                        : _bufferedBarColor;
            ...
            for (var i = 0; i < _numberOfBands; i++)
            {
                if (Active)
                {
                    var bandLevel = NormalizedBandLevels.GetArrayElementAtIndex(i).floatValue;
                    _columnHeight = MMMaths.Remap(bandLevel, 0f, 1f, 0f, _maxColumnHeight);
                    _barColor = Time.time - LastPeaksAt.GetArrayElementAtIndex(i).floatValue < _peakShowDuration
                        ? _peakColor
        ...
        protected virtual void DrawRawSpectrum()
        {
            ...
            {
                // spectrum
                for (var i = 1; i < RawSpectrum.arraySize - 1; i++)
                {
                    var xPosition = _spectrumBoxPosition.x + _externalMargin + MMMaths.Remap(i, 0,
                        RawSpectrum.arraySize, 0f, _spectrumBoxSize.x - _externalMargin * 2);
                    var yPosition = _spectrumBoxPosition.y + _spectrumBoxSize.y / 2;
                    var deltaX = (_spectrumBoxSize.x - _externalMargin * 2) / RawSpectrum.arraySize;

                    var spectrumValue = RawSpectrum.GetArrayElementAtIndex(i).floatValue;
                    var spectrumValuePrevious = RawSpectrum.GetArrayElementAtIndex(i - 1).floatValue;

                    var factor = _spectrumBoxSize.y / 2;
            ...
            else
            {
                ...
                {
                    var xPosition = _spectrumBoxPosition.x + _externalMargin +
                                    MMMaths.Remap(i, 0, points, 0f, _spectrumBoxSize.x - _externalMargin * 2);
                    var yPosition = _spectrumBoxPosition.y + _spectrumBoxSize.y / 2;
                    var deltaBetweenXandXPrevious = (_spectrumBoxSize.x - _externalMargin * 2) / points;

                    var spectrumValue = Foobar(i);
                    var spectrumValuePrevious = Foobar(i - 1);
