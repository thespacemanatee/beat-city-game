...
namespace MoreMountains.Tools
{
    ...
    public class MMPropertyPickerDrawer : PropertyDrawer
    {
        ...
        protected int _selectedComponentIndex;
        protected int _selectedPropertyIndex;
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            // rectangles
            var targetLabelRect = new Rect(position.x, position.y, position.width, _lineHeight);
            var targetObjectRect = new Rect(position.x, position.y + (_lineHeight + _lineMargin), position.width,
                _lineHeight);
            var targetComponentRect = new Rect(position.x, position.y + (_lineHeight + _lineMargin) * 2, position.width,
                _lineHeight);
            var targetPropertyRect = new Rect(position.x, position.y + (_lineHeight + _lineMargin) * 3, position.width,
                _lineHeight);
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                _TargetObject = property.FindPropertyRelative("TargetObject").objectReferenceValue;
                FillComponentsList(property);
        ...
        protected virtual void FillPropertyList(SerializedProperty property)
        {
            ...
            if (!_targetIsScriptableObject)
            {
                ...
                {
                    var newEntry = fieldInfo.Name + " [Field - " + fieldInfo.FieldType.Name + "]";
                ...
                foreach (var foundProperty in propertiesList)
                {
                    var newEntry = foundProperty.Name + " [Property - " + foundProperty.PropertyType.Name + "]";
                    tempPropertiesList.Add(newEntry);
        ...
        protected virtual void GetComponentIndex(SerializedProperty property)
        {
            ...
            var targetComponent = property.FindPropertyRelative("TargetComponent").objectReferenceValue as Component;
        ...
        protected virtual void GetPropertyIndex(SerializedProperty property)
        {
            ...
            var targetProperty = property.FindPropertyRelative("TargetPropertyName").stringValue;
