...
namespace MoreMountains.Tools
{
    ...
    public class MMExecutionOrderAttribute : Attribute
    {
        ...
        {
            foreach (var assemblyType in _assemblyTypes)
            {
                if (!HasExecutionOrderAttribute(assemblyType)) continue;

                var attributes = assemblyType.GetCustomAttributes(_executionOrderAttributeType, false);
                var attribute = attributes[0] as MMExecutionOrderAttribute;

                var asset = "";
                var guids = AssetDatabase.FindAssets(assemblyType.Name + " t:script");
                ...
                var monoScript = AssetDatabase.LoadAssetAtPath<MonoScript>(AssetDatabase.GUIDToAssetPath(asset));
        ...
        {
            var executionOrderHasChanged = false;
