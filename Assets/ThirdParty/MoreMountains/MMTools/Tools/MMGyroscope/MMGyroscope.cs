...
namespace MoreMountains.Tools
{
    ...
    public class MMGyroscope : MonoBehaviour
    {
        ...
        [Range(-1f, 1f)] public float TestXAcceleration;

        /// the rotation to apply on the y axis while in test mode
        [Range(-1f, 1f)] public float TestYAcceleration;

        /// the rotation to apply on the y axis while in test mode
        [Range(-1f, 1f)] public float TestZAcceleration;
        ...
        [SerializeField] private Vector2 _Clamps = new(-1f, 1f);
        ...
        {
            var deltaTime = TimeScale == TimeScales.Scaled ? Time.deltaTime : Time.unscaledDeltaTime;
        ...
        {
            var rotationQuaternion = Quaternion.FromToRotation(-Vector3.forward, initialAcceleration);
            var newMatrix = Matrix4x4.TRS(Vector3.zero, rotationQuaternion, Vector3.one);
            return newMatrix.inverse;
        }

        private static Vector3 CalibratedAcceleration(Vector3 accelerator, Matrix4x4 matrix)
        {
            var fixedAcceleration = matrix.MultiplyVector(accelerator);
