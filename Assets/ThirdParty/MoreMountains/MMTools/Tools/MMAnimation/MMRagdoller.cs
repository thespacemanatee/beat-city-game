...
namespace MoreMountains.Tools
{
    ...
    ///     If you have parts of your ragdoll that you don't want to be affected by this script (a weapon for example), just
    ///     add a MMRagdollerIgnore component to them
    /// </summary>
    ...
    public class MMRagdoller : MonoBehaviour
    {
        ...
        protected virtual void Initialization()
        {
            ...
            foreach (var component in transforms)
                ...
                {
                    var bodyPart = new RagdollBodyPart { BodyPartTransform = component as Transform };
        ...
        protected virtual void RegisterAnimatorParameters()
        {
            ...
            if (_animator.MMHasParameterOfType(_getUpFromBackAnimationParameterName,
                    AnimatorControllerParameterType.Bool)) _animatorParameters.Add(_getUpFromBackAnimationParameter);
            if (_animator.MMHasParameterOfType(_getUpFromBellyAnimationParameterName,
                    AnimatorControllerParameterType.Bool)) _animatorParameters.Add(_getUpFromBellyAnimationParameter);
        }
        ...
        protected virtual void HandleBlending()
        {
            if (CurrentState == RagdollStates.Blending)
            {
                ...
                {
                    var animatedToRagdolling =
                        _ragdolledHipPosition - _animator.GetBoneTransform(HumanBodyBones.Hips).position;
                    var newRootPosition = transform.position + animatedToRagdolling;
                    ...
                    var ragdollingDirection = _ragdolledHeadPosition - _ragdolledFeetPosition;
                    ragdollingDirection.y = 0;

                    var meanFeetPosition = 0.5f * (_animator.GetBoneTransform(HumanBodyBones.LeftFoot).position +
                                                   _animator.GetBoneTransform(HumanBodyBones.RightFoot).position);
                    var animatedDirection = _animator.GetBoneTransform(HumanBodyBones.Head).position - meanFeetPosition;
                ...
                var ragdollBlendAmount = 1.0f - (Time.time - _ragdollingEndTimestamp - _mecanimToGetUpTransitionTime) /
