...
{
    /// <summary>
    ///     This class will let you mirror the behaviour of an Animator's parameters on a Source Animator onto the ones of a
    ///     Target Animator.
    ///     Target will mirror Source.
    ///     Only the parameters existing on both Target and Source will be considered, you'll need to have the same on both
    ...
    public class MMAnimatorMirror : MonoBehaviour
    {
        ...
        {
            if (TargetAnimator == null) TargetAnimator = gameObject.GetComponent<Animator>();

            if (TargetAnimator == null || SourceAnimator == null) return;

            // we store our source parameters
            var numberOfParameters = SourceAnimator.parameterCount;
            _sourceParameters = new AnimatorControllerParameter[numberOfParameters];
            for (var i = 0; i < numberOfParameters; i++) _sourceParameters[i] = SourceAnimator.GetParameter(i);
            ...
            for (var i = 0; i < numberOfParameters; i++) _targetParameters[i] = TargetAnimator.GetParameter(i);

            // we store our matching parameters
            _updateParameters = new List<MMAnimatorMirrorBind>();

            foreach (var sourceParam in _sourceParameters)
            foreach (var targetParam in _targetParameters)
                if (sourceParam.name == targetParam.name)
                {
                    var bind = new MMAnimatorMirrorBind();
        ...
        protected virtual void Mirror()
        {
            ...
            foreach (var bind in _updateParameters)
