...
{
	/// <summary>
	///     A base class, meant to be extended depending on the use (simple, multiple object pooler), and used as an interface
	///     by the spawners.
	...
	public abstract class MMObjectPooler : MonoBehaviour
    {
        ...
        [MMCondition("NestWaitingPool", true)] public bool NestUnderThis;
        ...
        {
            if (_objectPool != null && NestUnderThis) RemovePool(_objectPool);
        }
        ...
        {
            if (_pools == null) _pools = new List<MMObjectPool>(_initialPoolsListCapacity);
        ...
        protected virtual bool CreateWaitingPool()
        {
            ...
            var objectPool = ExistingPool(DetermineObjectPoolName());
            if (objectPool != null)
            {
        ...
        public virtual MMObjectPool ExistingPool(string poolName)
        {
            ...
            if (_pools.Count == 0)
            {
                ...
                if (pools.Length > 0) _pools.AddRange(pools);
            }

            foreach (var pool in _pools)
                if (pool != null && pool.name == poolName && pool.gameObject.scene == gameObject.scene)
                    return pool;
            return null;
        }
        ...
        {
            if (NestWaitingPool && NestUnderThis && _waitingPool != null) _waitingPool.transform.SetParent(transform);
        }
