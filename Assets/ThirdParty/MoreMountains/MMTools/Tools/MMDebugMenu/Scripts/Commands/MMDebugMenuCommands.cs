...
namespace MoreMountains.Tools
{
    ...
    public class MMDebugMenuCommands : MonoBehaviour
    {
        ...
        {
            var message = "Time.time is " + Time.time;
            MMDebug.DebugLogTime(message);
        }
        ...
        public static void Help()
        {
            ...
            foreach (var method in MMDebug.Commands.OrderBy(m => m.Name))
                result += "\n- <color=#FFFFFF>" + method.Name + "</color>";
            MMDebug.DebugLogTime(result, "#FFC400");
        }
        ...
        {
            if (int.TryParse(args[1], out var vSyncCount))
            {
                QualitySettings.vSyncCount = vSyncCount;
                MMDebug.DebugLogTime("VSyncCount set to " + vSyncCount, "#FFC400");
            }
        }
        ...
        {
            if (int.TryParse(args[1], out var framerate))
            {
                Application.targetFrameRate = framerate;
                MMDebug.DebugLogTime("Framerate set to " + framerate, "#FFC400");
            }
        }
        ...
        {
            if (float.TryParse(args[1], NumberStyles.Float, CultureInfo.InvariantCulture, out var timescale))
            {
                Time.timeScale = timescale;
                MMDebug.DebugLogTime("Timescale set to " + timescale, "#FFC400");
            }
        }
        ...
        {
            if (int.TryParse(args[1], out var i1) && int.TryParse(args[2], out var i2))
            {
                string result;
                var biggest = i1 >= i2 ? i1 : i2;
