...
namespace MoreMountains.Tools
{
    ...
    public static class MMAchievementManager
    {
        ...
        public static List<MMAchievement> AchievementsList { get; private set; }
        ...
        {
            AchievementsList = new List<MMAchievement>();
            ...
            foreach (var achievement in achievementList.Achievements) AchievementsList.Add(achievement.Copy());
        }
        ...
        /// <summary>
        ///     Determines if the achievement manager contains an achievement of the specified ID. Returns it if found, otherwise
        ///     returns null
        /// </summary>
        ...
        {
            if (AchievementsList.Count == 0) return null;
        ...
        {
            DeterminePath();
            var serializedMMAchievementManager = (SerializedMMAchievementManager)MMSaveLoadManager.Load(
                typeof(SerializedMMAchievementManager), _saveFileName + _saveFileExtension, _saveFolderName);
        ...
        {
            DeterminePath();
            var serializedMMAchievementManager = new SerializedMMAchievementManager();
        ...
        {
            var tempFileName = !string.IsNullOrEmpty(_listID) ? _listID : _defaultFileName;
        ...
        {
            serializedAchievements.Achievements = new SerializedMMAchievement[AchievementsList.Count];

            for (var i = 0; i < AchievementsList.Count(); i++)
            {
                var newAchievement = new SerializedMMAchievement(AchievementsList[i].AchievementID,
                    AchievementsList[i].UnlockedStatus, AchievementsList[i].ProgressCurrent);
        ...
        public static void ExtractSerializedMMAchievementManager(SerializedMMAchievementManager serializedAchievements)
        {
            ...
            for (var i = 0; i < AchievementsList.Count(); i++)
            for (var j = 0; j < serializedAchievements.Achievements.Length; j++)
                if (AchievementsList[i].AchievementID == serializedAchievements.Achievements[j].AchievementID)
                {
                    AchievementsList[i].UnlockedStatus = serializedAchievements.Achievements[j].UnlockedStatus;
                    AchievementsList[i].ProgressCurrent = serializedAchievements.Achievements[j].ProgressCurrent;
                }
        }
    }
}