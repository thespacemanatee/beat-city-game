...
namespace MoreMountains.Tools
{
    ...
    public static class MMSpawnAround
    {
        ...
        public static Vector3 SpawnAroundPosition(MMSpawnAroundProperties props, Vector3 origin)
        {
            ...
            {
                var distance = Random.Range(props.MinimumSphereRadius, props.MaximumSphereRadius);
            ...
            {
                var randomX = Random.Range(props.MinimumCubeBaseSize.x, props.MaximumCubeBaseSize.x);
                newPosition.x = Random.Range(-randomX, randomX) / 2f;
                var randomY = Random.Range(props.MinimumCubeBaseSize.y, props.MaximumCubeBaseSize.y);
                newPosition.y = Random.Range(-randomY, randomY) / 2f;
                var randomZ = Random.Range(props.MinimumCubeBaseSize.z, props.MaximumCubeBaseSize.z);
                newPosition.z = Random.Range(-randomZ, randomZ) / 2f;
                newPosition = Vector3.Cross(newPosition, props.NormalToSpawnPlane);
            }

            var randomOffset = Random.Range(props.MinimumNormalAxisOffset, props.MaximumNormalAxisOffset);
            // we correct the position based on the NormalOffsetCurve
            if (props.UseNormalAxisOffsetCurve)
            {
                var normalizedOffset = 0f;
                if (randomOffset != 0)
                {
                ...
                var offset = props.NormalOffsetCurve.Evaluate(normalizedOffset);
