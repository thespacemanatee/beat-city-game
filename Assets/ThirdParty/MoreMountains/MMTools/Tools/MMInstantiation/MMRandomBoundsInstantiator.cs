...
namespace MoreMountains.Tools
{
    ...
    public class MMRandomBoundsInstantiator : MonoBehaviour
    {
        ...
        [MMVector("Min", "Max")] public Vector2Int Quantity = new(1, 1);
        ...
        protected virtual void Instantiate()
        {
            ...
            {
                foreach (var go in _instantiatedGameObjects) DestroyImmediate(go);
                _instantiatedGameObjects.Clear();
            }

            var random = Random.Range(Quantity.x, Quantity.y);
            for (var i = 0; i < random; i++) InstantiateRandomObject();
        }
        ...
        public virtual void InstantiateRandomObject()
        {
            ...
            var obj = Instantiate(RandomPool[randomIndex], transform.position, transform.rotation);
            ...
            if (ParentInstantiatedToThisObject) obj.transform.SetParent(transform);
