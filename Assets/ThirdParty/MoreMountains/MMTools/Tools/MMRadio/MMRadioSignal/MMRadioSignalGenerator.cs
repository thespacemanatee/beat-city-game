...
namespace MoreMountains.Tools
{
    ...
    ///     like a supercharged animation curve. In that case, simply disable the component, and read from it using its
    ///     Evaluate method
    /// </summary>
    public class MMRadioSignalGenerator : MMRadioSignal
    {
        ...
        [MMVector("Min", "Max")] public Vector2 Clamps = new(0f, 1f);
        ...
        public virtual float Evaluate(float time)
        {
            ...
            for (var i = 0; i < SignalList.Count; i++)
                if (SignalList[i].Active)
                {
                    var newLevel = MMSignal.GetValueNormalized(time,
        ...
        protected override void Shake()
        {
            ...
            {
                var elapsedTime = TimescaleTime - _shakeStartedTimestamp;
