        ...
        public virtual void MoveAlongThePath()
        {
            switch (AccelerationType)
            {
                ...
                case PossibleAccelerationType.AnimationCurve:
                    var distanceBetweenPoints = Vector3.Distance(_previousPoint, _currentPoint.Current);

                    if (distanceBetweenPoints <= 0) return;

                    var remappedDistance = 1 - MMMaths.Remap(_distanceToNextPoint, 0f, distanceBetweenPoints, 0f, 1f);
                    var speedFactor = Acceleration.Evaluate(remappedDistance);
