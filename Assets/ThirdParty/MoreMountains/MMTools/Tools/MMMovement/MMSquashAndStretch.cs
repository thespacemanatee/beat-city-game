...
namespace MoreMountains.Tools
{
    ...
    public class MMSquashAndStretch : MonoBehaviour
    {
        ...
        [MMVector("Min", "Max")] public Vector2 SquashDuration = new(0.25f, 0.5f);

        /// the maximum intensity of the squash
        [MMVector("Min", "Max")] public Vector2 SquashIntensity = new(0f, 1f);
        ...
        [MMCondition("Spring", true)] public float SpringDamping = 0.3f;

        /// the spring's frequency
        [MMCondition("Spring", true)] public float SpringFrequency = 3f;

        /// the speed of the spring
        [MMCondition("Spring", true)] public float SpringSpeed = 10f;
        ...
        protected virtual void ComputeVelocityAndDirection()
        {
            ...
            if (AutoSquashOnStop)
            {
                ...
                else if (_movementStarted)
                {
                    ...
                    _squashing = true;
                    var duration = MMMaths.Remap(_lastVelocity, 0f, MaximumVelocity, SquashDuration.x,
                        SquashDuration.y);
                    var intensity = MMMaths.Remap(_lastVelocity, 0f, MaximumVelocity, SquashIntensity.x,
                        SquashIntensity.y);
        ...
        protected virtual void ComputeNewLocalScale()
        {
            if (_squashing)
            {
                var elapsed = MMMaths.Remap(TimescaleTime - _squashStartedAt, 0f, _squashDuration, 0f, 1f);
