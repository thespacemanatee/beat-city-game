...
namespace MoreMountains.Tools
{
    ...
    public class Beat
    {
        ...
        [HideInInspector] public bool _initialized;
    ...
    ///     microphone), and will cut it into chunks that you can then use to emit beat events, that other objects can consume
    ///     and act upon.
    ///     The sample interval is the frequency at which sound will be analyzed, the amount of spectrum samples will determine
    ///     the
    ///     accuracy of the sampling, the window defines the method used to reduce leakage, and the number of bands
    ///     will determine in how many bands you want to cut the sound. The more bands, the more levers you'll have to play
    ///     with afterwards.
    ///     In general, for all of these settings, higher values mean better quality and lower performance. The buffer speed
    ///     determines how
    ...
    public class MMAudioAnalyzer : MonoBehaviour
    {
        ...
        [HideInInspector] public bool PeaksPasted;
        ...
        public virtual void Initialization()
        {
            ...
            {
#if !UNITY_WEBGL
                var audioSourceGo = new GameObject("Microphone");
        ...
        {
            var coefficient = Mathf.Log(RawSpectrum.Length);
            var offset = 0;
            for (var i = 0; i < BandLevels.Length; i++)
            {
                var savedSum = 0f;
                var next = Mathf.Exp(coefficient / BandLevels.Length * (i + 1));
                var weight = 1f / (next - offset);
                for (var sum = 0f; offset < next; offset++)
                {
        ...
        protected virtual void ComputeAmplitudes()
        {
            ...
            for (var i = 0; i < _cachedNumberOfBands; i++)
            {
