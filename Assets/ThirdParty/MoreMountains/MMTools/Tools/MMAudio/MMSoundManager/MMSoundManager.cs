...
{
    /// <summary>
    ///     A simple yet powerful sound manager, that will let you play sounds with an event based approach and performance in
    ///     mind.
    ///     Features :
    ///     - Play/stop/pause/resume/free sounds
    ///     - Full control : loop, volume, pitch, pan, spatial blend, bypasses, priority, reverb, doppler level, spread,
    ///     rolloff mode, distance
    ///     - 2D & 3D spatial support
    ///     - Built-in pooling, automatically recycle a set of audio sources for maximum performance
    ///     - Built in audio mixer and groups, with ready-made tracks (Master, Music, SFX, UI), and options to play on more
    ///     groups if needed
        ...
        MMEventListener<MMSoundManagerTrackFadeEvent>
    {
        ...
        )
        {
            ...
            var audioSource = recycleAudioSource;
            ...
            if (soloSingleTrack)
            {
                MuteSoundsOnTrack(mmSoundManagerTrack, true);
            ...
            float initialVolume, float finalVolume, MMTweenType tweenType)
        {
            ...
            while (Time.unscaledTime - startedAt <= duration)
            {
                ...
                var newVolume = MMTween.Tween(elapsedTime, 0f, duration, initialVolume, finalVolume, tweenType);
        ...
        {
            foreach (var sound in _sounds) sound.Source.Play();
        }
