...
namespace MoreMountains.TopDownEngine
{
    ...
    public class Teleporter : ButtonActivated
    {
        ...
        public enum TeleportationModes
        {
            Instant,
            Tween
        }
        ...
        public bool MaintainXEntryPositionOnExit;
        ...
        public bool MaintainYEntryPositionOnExit;
        ...
        public bool MaintainZEntryPositionOnExit;
        ...
        /// the target room
        [Tooltip("the target room")] public Room TargetRoom;
        ...
        [MMCondition("TriggerFade", true)] [Tooltip("the ID of the fader to target")]
        public int FaderID;
        ...
        public bool FadeIgnoresTimescale;
        ...
        [MMCondition("MoveMask", true)] [Tooltip("the curve to move the mask along to")]
        public MMTween.MMTweenCurve MoveMaskCurve = MMTween.MMTweenCurve.EaseInCubic;

        /// the method to move the mask
        [MMCondition("MoveMask", true)] [Tooltip("the method used to move the mask")]
        public MMSpriteMaskEvent.MMSpriteMaskEventTypes MoveMaskMethod =
            MMSpriteMaskEvent.MMSpriteMaskEventTypes.ExpandAndMoveToNewPosition;
        ...
        protected virtual void OnDrawGizmos()
        {
            if (Destination != null)
            {
                ...
                MMDebug.DrawGizmoArrow(transform.position,
                    Destination.transform.position + Destination.ExitOffset - transform.position, Color.cyan, 1f);
                // draws a point at the exit position 
                MMDebug.DebugDrawCross(transform.position + ExitOffset, 0.5f, Color.yellow);
                MMDebug.DrawPoint(transform.position + ExitOffset, Color.yellow, 0.5f);
            }

            if (TargetRoom != null)
                // draws an arrow to the destination room
                MMDebug.DrawGizmoArrow(transform.position, TargetRoom.transform.position - transform.position,
                    MMColors.Pink, 1f);
        }
        ...
        protected virtual void InitializeTeleporter()
        {
            ...
            if (CurrentRoom == null) CurrentRoom = gameObject.GetComponentInParent<Room>();
        }
        ...
        protected override void TriggerEnter(GameObject collider)
        {
            ...
            if (_ignoreList.Contains(collider.transform)) return;
        ...
        protected virtual void Teleport(GameObject collider)
        {
            ...
            if (Destination != null) StartCoroutine(TeleportSequence(collider));
        }
        ...
        protected virtual IEnumerator TeleportSequence(GameObject collider)
        {
            ...
            for (float timer = 0, duration = FadeOutDuration; timer < duration; timer += LocalDeltaTime)
                yield return null;

            AfterFadeOut(collider);

            for (float timer = 0, duration = DelayBetweenFades; timer < duration; timer += LocalDeltaTime)
                yield return null;

            AfterDelayBetweenFades(collider);

            for (float timer = 0, duration = FadeInDuration; timer < duration; timer += LocalDeltaTime)
                yield return null;
        ...
        protected virtual void SequenceStart(GameObject collider)
        {
            ...
            if (CameraMode == CameraModes.TeleportCamera) MMCameraEvent.Trigger(MMCameraEventTypes.StopFollowing);

            if (FreezeTime) MMTimeScaleEvent.Trigger(MMTimeScaleMethods.For, 0f, 0f, false, 0f, true);

            if (FreezeCharacter && _player != null) _player.Freeze();
        }
        ...
        {
            if (TriggerFade)
                MMFadeInEvent.Trigger(FadeOutDuration, FadeTween, FaderID, FadeIgnoresTimescale,
        ...
        protected virtual void AfterFadeOut(GameObject collider)
        {
            ...
            if (AddToDestinationIgnoreList) Destination.AddToIgnoreList(collider.transform);
            ...
            if (CurrentRoom != null) CurrentRoom.PlayerExitsRoom();

            if (TargetRoom != null)
            {
                TargetRoom.PlayerEntersRoom();
                TargetRoom.VirtualCamera.Priority = 10;
                MMSpriteMaskEvent.Trigger(MoveMaskMethod, TargetRoom.RoomColliderCenter, TargetRoom.RoomColliderSize,
                    MoveMaskDuration, MoveMaskCurve);
            }
        }
        ...
        protected virtual void TeleportCollider(GameObject collider)
        {
            ...
            if (MaintainXEntryPositionOnExit) _newPosition.x = _entryPosition.x;
            if (MaintainYEntryPositionOnExit) _newPosition.y = _entryPosition.y;
            if (MaintainZEntryPositionOnExit) _newPosition.z = _entryPosition.z;
        ...
        protected virtual IEnumerator TeleportTweenCo(GameObject collider, Vector3 origin, Vector3 destination)
        {
            ...
            {
                var elapsedTime = LocalTime - startedAt;
                collider.transform.position =
                    MMTween.Tween(elapsedTime, 0f, DelayBetweenFades, origin, destination, TweenCurve);
                yield return null;
            }
        ...
        protected virtual void AfterDelayBetweenFades(GameObject collider)
        {
            ...
            if (TriggerFade)
                MMFadeOutEvent.Trigger(FadeInDuration, FadeTween, FaderID, FadeIgnoresTimescale,
                    LevelManager.Instance.Players[0].transform.position);
        }
        ...
        {
            if (FreezeCharacter && _player != null) _player.UnFreeze();

            if (_characterGridMovement != null) _characterGridMovement.SetCurrentWorldPositionAsNewPosition();

            if (FreezeTime) MMTimeScaleEvent.Trigger(MMTimeScaleMethods.Unfreeze, 1f, 0f, false, 0f, false);
        }

        /// <summary>
        ///     When something exits the teleporter, if it's on the ignore list, we remove it from it, so it'll be considered next
        ///     time it enters.
        /// </summary>
        /// <param name="collider">Collider.</param>
        public override void TriggerExitAction(GameObject collider)
        {
            if (_ignoreList.Contains(collider.transform)) _ignoreList.Remove(collider.transform);
            base.TriggerExitAction(collider);
        }

        /// <summary>
        ///     Adds an object to the ignore list, which will prevent that object to be moved by the teleporter while it's in that
        ///     list
        /// </summary>
        /// <param name="objectToIgnore">Object to ignore.</param>
        public virtual void AddToIgnoreList(Transform objectToIgnore)
        {
            if (!_ignoreList.Contains(objectToIgnore)) _ignoreList.Add(objectToIgnore);
        }
    }
}