...
namespace MoreMountains.TopDownEngine
{
    ...
    ///     context.
    ///     Note that it's not the default solution in the engine at the moment, because older versions of Unity don't support
    ///     it,
    ...
    public class InputSystemManagerEventsBased : InputManager
    {
        ...
        {
            BindButton(context, JumpButton);
        }

        public void OnPrimaryMovement(InputAction.CallbackContext context)
        {
            _primaryMovement = context.ReadValue<Vector2>();
        }

        public void OnSecondaryMovement(InputAction.CallbackContext context)
        {
            _secondaryMovement = context.ReadValue<Vector2>();
        }

        public void OnRun(InputAction.CallbackContext context)
        {
            BindButton(context, RunButton);
        }

        public void OnDash(InputAction.CallbackContext context)
        {
            BindButton(context, DashButton);
        }

        public void OnCrouch(InputAction.CallbackContext context)
        {
            BindButton(context, CrouchButton);
        }

        public void OnShoot(InputAction.CallbackContext context)
        {
            BindButton(context, ShootButton);
        }

        public void OnSecondaryShoot(InputAction.CallbackContext context)
        {
            BindButton(context, SecondaryShootButton);
        }

        public void OnInteract(InputAction.CallbackContext context)
        {
            BindButton(context, InteractButton);
        }

        public void OnReload(InputAction.CallbackContext context)
        {
            BindButton(context, ReloadButton);
        }

        public void OnPause(InputAction.CallbackContext context)
        {
            BindButton(context, PauseButton);
        }

        public void OnSwitchWeapon(InputAction.CallbackContext context)
        {
            BindButton(context, SwitchWeaponButton);
        }

        public void OnSwitchCharacter(InputAction.CallbackContext context)
        {
            BindButton(context, SwitchCharacterButton);
        }

        public void OnTimeControl(InputAction.CallbackContext context)
        {
            BindButton(context, TimeControlButton);
        }
        ...
        protected virtual void BindButton(InputAction.CallbackContext context, MMInput.IMButton imButton)
        {
            ...
            {
                if (button.wasPressedThisFrame) imButton.State.ChangeState(MMInput.ButtonStates.ButtonDown);
                if (button.wasReleasedThisFrame) imButton.State.ChangeState(MMInput.ButtonStates.ButtonUp);
            }
        }
