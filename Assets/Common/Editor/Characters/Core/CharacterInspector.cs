...
namespace MoreMountains.TopDownEngine
{
    ...
    public class CharacterInspector : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (character.CharacterType == Character.CharacterTypes.Player) DrawDefaultInspector();

            // in AI mode draws everything but the PlayerID field
            if (character.CharacterType == Character.CharacterTypes.AI)
            {
                character.PlayerID = "";
                DrawPropertiesExcluding(serializedObject, "PlayerID");
            }

            EditorGUILayout.Space();
            EditorGUILayout.LabelField("Autobuild", EditorStyles.boldLabel);
            EditorGUILayout.HelpBox(
                "The Character Autobuild button will automatically add all the components needed for a functioning Character, and set their settings, layer, tags. Be careful, if you've already customized your character, this will reset its settings!",
        ...
        protected virtual void GenerateCharacter(Character.CharacterTypes type, Modes mode)
        {
            ...
            if (mode == Modes.TwoD)
            {
                ...
                var rigidbody2D = character.GetComponent<Rigidbody2D>() == null
                ...
                var sortingGroup = character.GetComponent<SortingGroup>() == null
                    ? character.gameObject.AddComponent<SortingGroup>()
                    : character.GetComponent<SortingGroup>();
                sortingGroup.sortingLayerName = "Characters";

                // Adds the boxcollider2D if needed
                var boxcollider2D = character.GetComponent<BoxCollider2D>() == null
                    ? character.gameObject.AddComponent<BoxCollider2D>()
                    : character.GetComponent<BoxCollider2D>();
                boxcollider2D.isTrigger = false;

                // adds the top down controller 2D
                var topDownController2D = character.GetComponent<TopDownController2D>() == null
                    ? character.gameObject.AddComponent<TopDownController2D>()
                    : character.GetComponent<TopDownController2D>();
            ...
            if (mode == Modes.ThreeD)
            {
                ...
                var characterController = character.GetComponent<CharacterController>() == null
                ...
                // adds a rigidbody
                var rigidbody = character.GetComponent<Rigidbody>() == null
                ...
                var topDownController3D = character.GetComponent<TopDownController3D>() == null
                    ? character.gameObject.AddComponent<TopDownController3D>()
                    : character.GetComponent<TopDownController3D>();
                topDownController3D.Gravity = 40;
                topDownController3D.ObstaclesLayerMask = LayerMask.GetMask("Obstacles", "Ground", "ObstaclesDoors",
                    "MovingPlatform", "FallingPlatform");
            ...
            // adds health
            var health = character.GetComponent<Health>() == null
