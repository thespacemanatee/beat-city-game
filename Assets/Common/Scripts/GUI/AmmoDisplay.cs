...
namespace MoreMountains.TopDownEngine
{
    ...
    public class AmmoDisplay : MMProgressBar
    {
        ...
        {
            if (TextDisplay != null) TextDisplay.text = newText;
        }
        ...
        public virtual void UpdateAmmoDisplays(bool magazineBased, int totalAmmo, int maxAmmo, int ammoInMagazine,
            int magazineSize, bool displayTotal)
        {
            ...
            if (magazineBased)
            {
                UpdateBar(ammoInMagazine, 0, magazineSize);
                if (displayTotal)
                {
                    ...
                    UpdateTextDisplay(_stringBuilder.ToString());
                }
                else
                {
                    _stringBuilder.Append(ammoInMagazine.ToString());
