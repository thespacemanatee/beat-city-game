...
namespace MoreMountains.TopDownEngine
{
    ...
    public class MovingPlatform2D : MMPathMovement
    {
        ...
        public bool UseSafeDistance;

        /// the distance to move the character at in safe distance mode
        [MMCondition("UseSafeDistance", true)] [Tooltip("the distance to move the character at in safe distance mode")]
        ...
        protected virtual void AttachCharacterToMovingPlatform(Collider2D collider)
        {
            ...
            if (_topdDownController2D != null) _topdDownController2D.SetMovingPlatform(this);
            // 
            ...
            if (UseSafeDistance)
            {
                ...
                {
                    _translationVector = (transform.position - collider.transform.position).normalized *
                                         Mathf.Min(distance, ForcedSafeDistance);
        ...
        {
            _topdDownController2D = collider.gameObject.MMGetComponentNoAlloc<TopDownController2D>();
            if (_topdDownController2D != null) _topdDownController2D.SetMovingPlatform(null);
        }
    }
}