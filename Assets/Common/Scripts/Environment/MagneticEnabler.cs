...
{
    /// <summary>
    ///     This object will enable Magnetic objects in your scene when they enter its associated collider2D (make sure you add
    ///     one).
    ///     While magnetic objects can work on their own, and handle their own range detection, you can also use a different
    ///     architecture, where an enabler makes them move.
    ...
    public class MagneticEnabler : MonoBehaviour
    {
        ...
        public bool OverrideFollowPositionSpeed;

        /// the value with which to override the speed
        [Tooltip("the speed with which to override the speed")] [MMCondition("OverrideFollowPositionSpeed", true)]
        ...
        public bool OverrideFollowAcceleration;

        /// the value with which to override the acceleration
        [Tooltip("the speed with which to override the acceleration")] [MMCondition("OverrideFollowAcceleration", true)]
        ...
        protected virtual void OnTriggerEnterInternal(GameObject colliding)
        {
            ...
            if (_magnetic == null) return;

            var idFound = false;
            if (_magnetic.MagneticTypeID == 0)
                idFound = true;
            else
                foreach (var id in MagneticTypeIDs)
                    if (id == _magnetic.MagneticTypeID)
                        idFound = true;

            if (!idFound) return;

            if (OverrideFollowAcceleration) _magnetic.FollowAcceleration = FollowAcceleration;

            if (OverrideFollowPositionSpeed) _magnetic.FollowPositionSpeed = FollowPositionSpeed;

            _magnetic.SetTarget(transform);
