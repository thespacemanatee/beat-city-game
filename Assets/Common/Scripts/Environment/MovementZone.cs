...
{
    /// <summary>
    ///     Add this class to a trigger and it will let you apply a speed multiplier to characters entering it
    /// </summary>
    ...
    public class MovementZone : ButtonActivated
    {
        ...
        {
            if (_characterButtonActivation == null) return;

            var collidingEntity = new MovementZoneCollidingEntity();
            ...
            collidingEntity.TargetCharacterMovement = _characterButtonActivation.gameObject
                .GetComponentInParent<Character>()?.FindAbility<CharacterMovement>();
            collidingEntity.MultiplierStorage = collidingEntity.TargetCharacterMovement.ContextSpeedMultiplier;

            CollidingEntities.Add(collidingEntity);

            collidingEntity.TargetCharacterMovement.ContextSpeedMultiplier = MovementSpeedMultiplier;
        }
        ...
        {
            foreach (var entity in CollidingEntities)
                if (entity.TargetCharacter.gameObject == collider)
                {
                    entity.TargetCharacterMovement.ContextSpeedMultiplier = entity.MultiplierStorage;
        ...
        {
            public float MultiplierStorage;
