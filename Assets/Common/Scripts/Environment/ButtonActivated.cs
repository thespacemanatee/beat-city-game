...
namespace MoreMountains.TopDownEngine
{
    ...
    public class ButtonActivated : MonoBehaviour
    {
        ...
        [Header("Requirements")]
        [MMInformation(
            "Here you can specify what is needed for something to interact with this zone. Does it require the ButtonActivation character ability? Can it only be interacted with by the Player? ",
        ...
        [MMInformation(
            "Here you can specific how that zone is interacted with. You can have it auto activate, activate only when grounded, or prevent its activation altogether.",
            MMInformationAttribute.InformationType.Info, false)]
        ...
        public bool CanOnlyActivateIfGrounded;
        ...
        [MMInformation(
            "You can decide to have that zone be interactable forever, or just a limited number of times, and can specify a delay between uses (in seconds).",
            MMInformationAttribute.InformationType.Info, false)]
        ...
        [MMEnumCondition("InputType", (int)InputTypes.Key)] [Tooltip("the key used to activate this zone")]
        ...
        [MMInformation("You can have this zone show a visual prompt to indicate to the player that it's interactable.",
            MMInformationAttribute.InformationType.Info, false)]
        ...
        [MMCondition("UseVisualPrompt", true)] [Tooltip("the gameobject to instantiate to present the prompt")]
        public ButtonPrompt ButtonPromptPrefab;

        /// the text to display in the button prompt
        [MMCondition("UseVisualPrompt", true)] [Tooltip("the text to display in the button prompt")]
        ...
        [MMCondition("UseVisualPrompt", true)] [Tooltip("the color for the prompt's text")]
        ...
        [MMCondition("UseVisualPrompt", true)] [Tooltip("If true, the prompt will hide after use")]
        ...
        [MMCondition("UseVisualPrompt", true)] [Tooltip("the rotation of the actual buttonA prompt ")]
        ...
        protected bool _promptHiddenForever;
        protected bool _staying;
        ...
        {
            if (_staying && OnStay != null) OnStay.Invoke();
        }
        ...
        public virtual void Initialization()
        {
            ...
            DeniedFeedback?.Initialization(gameObject);
            EnterFeedback?.Initialization(gameObject);
            ExitFeedback?.Initialization(gameObject);
        ...
        /// <summary>
        ///     When the input button is pressed, we check whether or not the zone can be activated, and if yes, trigger
        ///     ZoneActivated
        /// </summary>
        ...
        protected virtual void ActivateZone()
        {
            ...
            if (DisableAfterUse && _numberOfActivationsLeft <= 0) DisableZone();
        }
        ...
        {
            if (_buttonPromptAnimator != null) _buttonPromptAnimator.SetTrigger("Error");
            DeniedFeedback?.PlayFeedbacks(transform.position);
        }
        ...
        {
            if (!UseVisualPrompt || _promptHiddenForever || ButtonPromptPrefab == null) return;

            // we add a blinking A prompt to the top of the zone
            if (_buttonPrompt == null)
            {
                _buttonPrompt = Instantiate(ButtonPromptPrefab);
                _buttonPrompt.Initialization();
                _buttonPromptAnimator = _buttonPrompt.gameObject.MMGetComponentNoAlloc<Animator>();
            }

            if (_collider != null) _buttonPrompt.transform.position = _collider.bounds.center + PromptRelativePosition;
            if (_collider2D != null)
        ...
        {
            if (!CheckConditions(collider)) return;
            ...
            if (CanOnlyActivateIfGrounded)
                if (collider != null)
                {
                    var controller = collider.gameObject.MMGetComponentNoAlloc<TopDownController>();
                    if (controller != null)
            ...
            if (!TestForLastObject(collider)) return;

            EnterFeedback?.PlayFeedbacks(transform.position);

            if (ShouldUpdateState)
            {
                _characterButtonActivation = collider.gameObject.MMGetComponentNoAlloc<Character>()
                    ?.FindAbility<CharacterButtonActivation>();
        ...
        protected virtual void TriggerExit(GameObject collider)
        {
            ...
            AutoActivationInProgress = false;
            if (_autoActivationCoroutine != null) StopCoroutine(_autoActivationCoroutine);
            ...
            ExitFeedback?.PlayFeedbacks(transform.position);

            if (_buttonPrompt != null && !AlwaysShowPrompt) HidePrompt();
        ...
        public virtual bool CheckNumberOfUses()
        {
            ...
            if (Time.time - _lastActivationTimestamp < DelayBetweenUses) return false;

            if (UnlimitedActivations) return true;

            if (_numberOfActivationsLeft == 0) return false;

            if (_numberOfActivationsLeft > 0) return true;
            return false;
        }
        ...
        {
            var character = collider.gameObject.MMGetComponentNoAlloc<Character>();
            ...
            switch (ButtonActivatedRequirement)
            {
                ...
                case ButtonActivatedRequirements.ButtonActivator:
                    if (collider.gameObject.MMGetComponentNoAlloc<ButtonActivator>() == null) return false;
                    break;
            ...
            {
                if (character == null) return false;
                if (character.CharacterType != Character.CharacterTypes.Player) return false;
            }

            if (RequiresButtonActivationAbility)
            {
                var characterButtonActivation = collider.gameObject.MMGetComponentNoAlloc<Character>()
                    ?.FindAbility<CharacterButtonActivation>();
