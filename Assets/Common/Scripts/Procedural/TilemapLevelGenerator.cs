...
{
    /// <summary>
    ///     This component, added on an empty object in your level will handle the generation of a unique and randomized
    ///     tilemap
    /// </summary>
    public class TilemapLevelGenerator : MMTilemapGenerator
    {
        ...
        {
            var boxCollider = TargetLevelManager.GetComponent<BoxCollider>();

            var bounds = ObstaclesTilemap.localBounds;
        ...
        protected virtual void PlaceEntryAndExit()
        {
            ...
            Vector3 spawnPosition = MMTilemap.GetRandomPosition(ObstaclesTilemap, TargetGrid, width, height, false,
                width * height * 2);
            ...
            while (Vector3.Distance(exitPosition, spawnPosition) < MinDistanceFromSpawnToExit &&
                   iterationsCount < _maxIterationsCount)
            {
                exitPosition = MMTilemap.GetRandomPosition(ObstaclesTilemap, TargetGrid, width, height, false,
                    width * height * 2);
        ...
        {
            if (WallsShadowTilemap != null) WallsShadowTilemap.UpdateShadows();
        }
    }
}