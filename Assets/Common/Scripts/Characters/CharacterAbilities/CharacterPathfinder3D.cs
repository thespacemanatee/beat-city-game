...
namespace MoreMountains.TopDownEngine
{
    ...
    public class CharacterPathfinder3D : CharacterAbility
    {
        ...
        protected override void Initialization()
        {
            ...
            _lastValidTargetPosition = transform.position;
        ...
        public virtual void SetNewDestination(Transform destinationTransform)
        {
            ...
            DeterminePath(transform.position, destinationTransform.position);
        }
        ...
        protected virtual void MoveController()
        {
            ...
            _direction = (Waypoints[NextWaypointIndex] - transform.position).normalized;
        ...
        protected virtual void DeterminePath(Vector3 startingPosition, Vector3 targetPosition)
        {
            ...
            if (_waypoints >= 2) NextWaypointIndex = 1;

            OnPathProgress?.Invoke(NextWaypointIndex, Waypoints.Length,
                Vector3.Distance(transform.position, Waypoints[NextWaypointIndex]));
        }
        ...
        {
            if (_waypoints <= 0) return;
            if (NextWaypointIndex < 0) return;

            var distance = Vector3.Distance(transform.position, Waypoints[NextWaypointIndex]);
        ...
        protected virtual void DetermineDistanceToNextWaypoint()
        {
            if (NextWaypointIndex <= 0)
                ...
                DistanceToNextWaypoint = Vector3.Distance(transform.position, Waypoints[NextWaypointIndex]);
        }
        ...
        protected virtual void DrawDebugPath()
        {
            if (DebugDrawPath)
            {
                ...
                for (var i = 0; i < _waypoints - 1; i++) Debug.DrawLine(Waypoints[i], Waypoints[i + 1], Color.red);
            }
        }
    }
}