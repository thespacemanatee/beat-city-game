...
namespace MoreMountains.TopDownEngine
{
    ...
    public class CharacterAbility : MonoBehaviour
    {
            ...
            get
            {
                if (_character != null)
                {
                    if (BlockingMovementStates != null && BlockingMovementStates.Length > 0)
                        for (var i = 0; i < BlockingMovementStates.Length; i++)
                    ...
                    if (BlockingConditionStates != null && BlockingConditionStates.Length > 0)
                        for (var i = 0; i < BlockingConditionStates.Length; i++)
        ...
        {
            if (_health == null) _health = gameObject.GetComponentInParent<Character>().CharacterHealth;

            if (_health == null) _health = gameObject.GetComponentInParent<Health>();
        ...
        protected virtual void Initialization()
        {
            ...
            _controller2D = gameObject.GetComponentInParent<TopDownController2D>();
            _controller3D = gameObject.GetComponentInParent<TopDownController3D>();
        ...
        {
            if (_character._animator == null) _character.AssignAnimator();

            _animator = _character._animator;

            if (_animator != null) InitializeAnimatorParameters();
        }
        ...
        /// <summary>
        ///     Called at the very start of the ability's cycle, and intended to be overridden, looks for input and calls methods
        ...
        /// <summary>
        ///     Override this to reset this ability's parameters. It'll be automatically called when the character gets killed, in
        ///     anticipation for its respawn.
        /// </summary>
            ...
            if (AbilityInProgressSfx != null)
                if (_abilityInProgressSfx == null)
                    _abilityInProgressSfx = MMSoundManagerSoundPlayEvent.Trigger(AbilityInProgressSfx,
                        MMSoundManager.MMSoundManagerTracks.Sfx, transform.position, true);
        }
