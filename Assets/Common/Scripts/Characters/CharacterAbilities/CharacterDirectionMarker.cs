...
namespace MoreMountains.TopDownEngine
{
    ...
    public class CharacterDirectionMarker : CharacterAbility
    {
        ...
        [Tooltip(
            "whether or not to offset the position along the direction's magnitude (for example, moving faster could move the marker further away from the character)")]
        public bool OffsetAlongMagnitude;

        /// the minimum bounds of the velocity's magnitude
        [Tooltip("the minimum bounds of the velocity's magnitude")] [MMCondition("OffsetAlongMagnitude", true)]
        public float MinimumVelocity;

        /// the maximum bounds of the velocity's magnitude
        [Tooltip("the maximum bounds of the velocity's magnitude")] [MMCondition("OffsetAlongMagnitude", true)]
        ...
        public bool DisableBelowThreshold;

        /// the threshold below which to disable the marker
        [Tooltip("the threshold below which to disable the marker")] [MMCondition("DisableBelowThreshold", true)]
        ...
        [Tooltip("the rate at which to interpolate the rotation")] [MMCondition("Interpolate", true)]
        public float InterpolateRate = 5f;

        [Header("Interpolation")] protected CharacterHandleWeapon _characterHandleWeapon;
        ...
        protected virtual void AimAt(Vector3 direction)
        {
            ...
            _newRotation = Quaternion.Slerp(transform.rotation, Quaternion.LookRotation(_direction, UpVector),
                InterpolateRate * Time.time);
        ...
        protected virtual void ApplyOffset(float rawValue)
        {
            ...
            {
                var remappedValue = MMMaths.Remap(rawValue, MinimumVelocity, MaximumVelocity, OffsetRemapMin,
                    OffsetRemapMax);
            ...
            if (Interpolate)
                _newPosition = MMMaths.Lerp(DirectionMarker.transform.localPosition, _newPosition, InterpolateRate,
                    Time.deltaTime);

            DirectionMarker.transform.localPosition = _newPosition;

            if (DisableBelowThreshold) DirectionMarker.gameObject.SetActive(rawValue > DisableThreshold);
        }
