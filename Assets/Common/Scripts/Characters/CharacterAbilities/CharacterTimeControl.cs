...
namespace MoreMountains.TopDownEngine
{
    ...
    public class CharacterTimeControl : CharacterAbility
    {
        ...
        [Tooltip("the duration for which to keep the timescale changed")] [MMEnumCondition("Mode", (int)Modes.OneTime)]
        ...
        protected override void HandleInput()
        {
            ...
            if (_inputManager.TimeControlButton.State.CurrentState == MMInput.ButtonStates.ButtonDown)
                TimeControlStart();
            if (_inputManager.TimeControlButton.State.CurrentState == MMInput.ButtonStates.ButtonUp) TimeControlStop();
        }
        ...
        public virtual void TimeControlStart()
        {
            if (Cooldown.Ready())
            {
                ...
                {
                    MMTimeScaleEvent.Trigger(MMTimeScaleMethods.For, TimeScale, Cooldown.ConsumptionDuration,
                        LerpTimeScale, LerpSpeed, true);
                    Cooldown.Start();
                    _timeControlled = true;
                }
                else
                {
                    MMTimeScaleEvent.Trigger(MMTimeScaleMethods.For, TimeScale, OneTimeDuration, LerpTimeScale,
                        LerpSpeed, false);
                    Cooldown.Start();
                }
            }
