...
{
    /// <summary>
    ///     Add this ability to a character and it'll rotate or flip to face the direction of movement or the weapon's, or
    ///     both, or none
    ...
    public class CharacterOrientation2D : CharacterAbility
    {
        ...
        [MMInformation(
            "You can also decide if the character must automatically flip when going backwards or not. Additionnally, if you're not using sprites, you can define here how the character's model's localscale will be affected by flipping. By default it flips on the x axis, but you can change that to fit your model.",
        ...
        [MMCondition("ModelShouldFlip", true)] [Tooltip("the scale value to apply to the model when facing left")]
        public Vector3 ModelFlipValueLeft = new(-1, 1, 1);

        /// the scale value to apply to the model when facing right
        [MMCondition("ModelShouldFlip", true)] [Tooltip("the scale value to apply to the model when facing right")]
        public Vector3 ModelFlipValueRight = new(1, 1, 1);
        ...
        public Vector3 ModelRotationValueLeft = new(0f, 180f, 0f);

        /// the rotation to apply to the model when it changes direction
        [MMCondition("ModelShouldRotate", true)]
        [Tooltip("the rotation to apply to the model when it changes direction")]
        public Vector3 ModelRotationValueRight = new(0f, 0f, 0f);
        ...
        [MMInformation(
            "It's usually good practice to build all your characters facing right. If that's not the case of this character, select Left instead.",
            MMInformationAttribute.InformationType.Info, false)]
        ...
        [MMReadOnly] [Tooltip("the direction this character is currently facing")]
        ...
        [MMReadOnly] [Tooltip("whether or not this character is facing right")]
        ...
        {
            base.Awake();
            _characterHandleWeapon = gameObject.GetComponentInParent<CharacterHandleWeapon>();
        }
        ...
        {
            base.Initialization();
            if (_controller == null) _controller = gameObject.GetComponentInParent<TopDownController>();
        ...
        public override void ProcessAbility()
        {
            ...
            _lastNonNullXMovement = Mathf.Abs(_controller.CurrentDirection.x) > 0
                ? _controller.CurrentDirection.x
                : _lastNonNullXMovement;
        }

        protected virtual void DetermineFacingDirection()
        {
            if (_controller.CurrentDirection == Vector3.zero) ApplyCurrentDirection();
            ...
            if (_controller.CurrentDirection.normalized.magnitude >= AbsoluteThresholdMovement)
            {
                if (Mathf.Abs(_controller.CurrentDirection.y) > Mathf.Abs(_controller.CurrentDirection.x))
                    CurrentFacingDirection = _controller.CurrentDirection.y > 0
                        ? Character.FacingDirections.North
                        : Character.FacingDirections.South;
                else
                    CurrentFacingDirection = _controller.CurrentDirection.x > 0
                        ? Character.FacingDirections.East
                        : Character.FacingDirections.West;
                _horizontalDirection = Mathf.Abs(_controller.CurrentDirection.x) >= AbsoluteThresholdMovement
                    ? _controller.CurrentDirection.x
                    : 0f;
                _verticalDirection = Mathf.Abs(_controller.CurrentDirection.y) >= AbsoluteThresholdMovement
                    ? _controller.CurrentDirection.y
                    : 0f;
            }
            else
            {
        ...
        protected virtual void ApplyModelRotation()
        {
            ...
            if (ModelRotationSpeed > 0f)
                _character.CharacterModel.transform.localEulerAngles = Vector3.Lerp(
        ...
        protected virtual void FlipToFaceMovementDirection()
        {
            ...
            if (_controller.CurrentDirection.normalized.magnitude >= AbsoluteThresholdMovement)
            {
                var checkedDirection = Mathf.Abs(_controller.CurrentDirection.normalized.x) > 0
                    ? _controller.CurrentDirection.normalized.x
                    : _lastNonNullXMovement;
        ...
        protected virtual void FlipToFaceWeaponDirection()
        {
            ...
            {
                var weaponAngle = _characterHandleWeapon.WeaponAimComponent.CurrentAngleAbsolute;
        ...
        public virtual void Face(Character.FacingDirections direction)
        {
            ...
            if (direction == Character.FacingDirections.West) FaceDirection(-1);
        }
        ...
        {
            if (ModelShouldFlip) FlipModel(direction);

            if (ModelShouldRotate) RotateModel(direction);
        ...
        {
            if (_character.CharacterModel != null)
                _character.CharacterModel.transform.localScale =
                    direction == 1 ? ModelFlipValueRight : ModelFlipValueLeft;
            else
        ...
        {
            if (_directionLastFrame != 0 && _directionLastFrame != _direction) _character.FlipAllAbilities();
        }
        ...
        {
            if (Time.deltaTime != 0f) _newSpeed = (transform.position - _positionLastFrame) / Time.deltaTime;
        ...
        {
            RegisterAnimatorParameter(_horizontalDirectionAnimationParameterName, AnimatorControllerParameterType.Float,
                out _horizontalDirectionAnimationParameter);
            RegisterAnimatorParameter(_verticalDirectionAnimationParameterName, AnimatorControllerParameterType.Float,
                out _verticalDirectionAnimationParameter);

            RegisterAnimatorParameter(_horizontalSpeedAnimationParameterName, AnimatorControllerParameterType.Float,
                out _horizontalSpeedAnimationParameter);
            RegisterAnimatorParameter(_verticalSpeedAnimationParameterName, AnimatorControllerParameterType.Float,
                out _verticalSpeedAnimationParameter);
            RegisterAnimatorParameter(_facingDirectionAnimationParameterName, AnimatorControllerParameterType.Float,
                out _facingDirectionAnimationParameter);
        }
        ...
        {
            MMAnimatorExtensions.UpdateAnimatorFloat(_animator, _horizontalDirectionAnimationParameter,
                _horizontalDirection, _character._animatorParameters, _character.RunAnimatorSanityChecks);
            MMAnimatorExtensions.UpdateAnimatorFloat(_animator, _verticalDirectionAnimationParameter,
                _verticalDirection, _character._animatorParameters, _character.RunAnimatorSanityChecks);

            MMAnimatorExtensions.UpdateAnimatorFloat(_animator, _horizontalSpeedAnimationParameter, _newSpeed.x,
                _character._animatorParameters, _character.RunAnimatorSanityChecks);
            MMAnimatorExtensions.UpdateAnimatorFloat(_animator, _verticalSpeedAnimationParameter, _newSpeed.y,
                _character._animatorParameters, _character.RunAnimatorSanityChecks);
            MMAnimatorExtensions.UpdateAnimatorFloat(_animator, _facingDirectionAnimationParameter, _directionFloat,
