...
namespace MoreMountains.TopDownEngine
{
    ...
    public class CharacterAbilityNodeSwap : CharacterAbility
    {
        ...
        ///     This ability reuses the SwitchCharacter input to avoid multiplying input entries, but feel free to override this
        ///     method to add a dedicated one
        /// </summary>
        ...
        {
            foreach (var node in _character.AdditionalAbilityNodes) node.gameObject.SetActive(false);

            _character.AdditionalAbilityNodes = AdditionalAbilityNodes;

            foreach (var node in _character.AdditionalAbilityNodes) node.gameObject.SetActive(true);
