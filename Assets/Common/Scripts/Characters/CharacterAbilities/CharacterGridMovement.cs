...
namespace MoreMountains.TopDownEngine
{
    ...
    public class CharacterGridMovement : CharacterAbility
    {
        ...
        [MMReadOnly] [Tooltip("the current speed at which the character is going")]
        public float CurrentSpeed;

        /// a multiplier to apply to the maximum speed
        [MMReadOnly] [Tooltip("a multiplier to apply to the maximum speed")]
        ...
        [MMReadOnly] [Tooltip("a multiplier to apply to the acceleration, letting you modify it safely from outside")]
        ...
        public Vector3 ObstacleDetectionOffset = new(0f, 0.5f, 0f);

        /// this is true everytime a character is at the exact position of a tile
        [MMReadOnly] [Tooltip("this is true everytime a character is at the exact position of a tile")]
        public bool PerfectTile;

        /// the coordinates of the cell this character currently occupies
        [MMReadOnly] [Tooltip("the coordinates of the cell this character currently occupies")]
        public Vector3Int CurrentCellCoordinates;

        /// whether this character is in 2D or 3D. This gets automatically computed at start
        [MMReadOnly] [Tooltip("whether this character is in 2D or 3D. This gets automatically computed at start")]
        public DimensionModes DimensionMode = DimensionModes.TwoD;

        [Header("Test")] [MMInspectorButton("Left")]
        public bool LeftButton;

        [MMInspectorButton("Right")] public bool RightButton;

        [MMInspectorButton("Up")] public bool UpButton;

        [MMInspectorButton("Down")] public bool DownButton;

        [MMInspectorButton("StopMovement")] public bool StopButton;

        [MMInspectorButton("LeftOneCell")] public bool LeftOneCellButton;

        [MMInspectorButton("RightOneCell")] public bool RightOneCellButton;

        [MMInspectorButton("UpOneCell")] public bool UpOneCellButton;

        [MMInspectorButton("DownOneCell")] public bool DownOneCellButton;
        ...
        protected bool _downPressedLastFrame;
        protected Vector3 _endWorldPosition;
        protected bool _firstPositionRegistered;
        ...
        protected bool _leftPressedLastFrame;
        protected bool _movementInterruptionBuffered;
        protected bool _movingToNextGridUnit;
        protected Vector3Int _newCellCoordinates = Vector3Int.zero;
        protected GridDirections _newDirection;
        protected bool _perfectTile;
        protected bool _rightPressedLastFrame;
        protected int _speedAnimationParameter;
        protected bool _stopBuffered;
        protected bool _upPressedLastFrame;
        ...
        protected virtual void RegisterFirstPosition()
        {
            ...
            {
                _endWorldPosition = transform.position;
                ...
                GridManager.Instance.SetLastPosition(gameObject, _lastOccupiedCellCoordinates);
                GridManager.Instance.SetNextPosition(gameObject, _lastOccupiedCellCoordinates);
        ...
        public override void ProcessAbility()
        {
            ...
            if (_character.ConditionState.CurrentState == CharacterStates.CharacterConditions.Frozen) return;
        ...
        {
            if (direction == GridDirections.None) return;
        ...
        protected virtual void HandleMovement()
        {
            ...
            if (!_movingToNextGridUnit)
            {
                ...
                if (_bufferedDirection == GridDirections.None)
                {
                    ...
                    GridManager.Instance.SetLastPosition(gameObject,
                        GridManager.Instance.WorldToCellCoordinates(_endWorldPosition));
                    GridManager.Instance.SetNextPosition(gameObject,
                        GridManager.Instance.WorldToCellCoordinates(_endWorldPosition));
                ...
                else
                {
                    GridManager.Instance.FreeCell(_lastOccupiedCellCoordinates);
                    GridManager.Instance.SetLastPosition(gameObject, _lastOccupiedCellCoordinates);
                    GridManager.Instance.SetNextPosition(gameObject, TargetGridPosition);
            ...
            var newPosition = Vector3.MoveTowards(transform.position, _endWorldPosition, Time.deltaTime * CurrentSpeed);

            _lastCurrentDirection = _endWorldPosition - transform.position;
            _lastCurrentDirection = _lastCurrentDirection.MMRound();
            if (_lastCurrentDirection != Vector3.zero) _controller.CurrentDirection = _lastCurrentDirection;
