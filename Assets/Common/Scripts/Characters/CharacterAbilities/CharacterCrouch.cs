...
namespace MoreMountains.TopDownEngine
{
    ...
    public class CharacterCrouch : CharacterAbility
    {
        ...
        public bool ResizeColliderWhenCrouched;
        ...
        [MMReadOnly] [Tooltip("whether or not the character is in a tunnel right now and can't get up")]
        ...
        {
            return
                "This component handles crouch and crawl behaviours. Here you can determine the crouch speed, and whether or not the collider should resize when crouched (to crawl into tunnels for example). If it should, please setup its new size here.";
        }
        ...
        protected override void Initialization()
        {
            ...
            if (ObjectsToOffset.Count > 0)
            {
                ...
                foreach (var go in ObjectsToOffset)
                    if (go != null)
        ...
        protected virtual void HandleForcedCrouch()
        {
            if (ForcedCrouch && _movement.CurrentState != CharacterStates.MovementStates.Crouching &&
                _movement.CurrentState != CharacterStates.MovementStates.Crawling) Crouch();
        }
        ...
        protected override void HandleInput()
        {
            ...
            if (_inputManager.CrouchButton.State.CurrentState == MMInput.ButtonStates.ButtonDown) Crouch();
        }
        ...
        protected virtual void Crouch()
        {
            ...
            if (_movement.CurrentState == CharacterStates.MovementStates.Running) _characterRun.RunStop();
            ...
            if (ResizeColliderWhenCrouched) _controller.ResizeColliderHeight(CrouchedColliderHeight);

            // we change our character's speed
            if (_characterMovement != null) _characterMovement.MovementSpeed = CrawlSpeed;

            // we prevent movement if we can't crawl
            if (!CrawlAuthorized) _characterMovement.MovementSpeed = 0f;
        }
        ...
        protected virtual void OffsetObjects()
        {
            ...
            if (ObjectsToOffset.Count > 0)
                for (var i = 0; i < ObjectsToOffset.Count; i++)
                {
                    var newOffset = Vector3.zero;
                    if (_movement.CurrentState == CharacterStates.MovementStates.Crouching) newOffset = OffsetCrouch;
                    if (_movement.CurrentState == CharacterStates.MovementStates.Crawling) newOffset = OffsetCrawl;
                    if (ObjectsToOffset[i] != null)
                        ObjectsToOffset[i].transform.localPosition = Vector3.Lerp(
                            ObjectsToOffset[i].transform.localPosition,
                            _objectsToOffsetOriginalPositions[i] + newOffset, Time.deltaTime * OffsetSpeed);
                }
        }
        ...
        protected virtual void ExitCrouch()
        {
            ...
            if (_characterMovement != null) _characterMovement.ResetSpeed();
