...
namespace MoreMountains.TopDownEngine
{
    ...
    public class CharacterJump3D : CharacterAbility
    {
        ...
        public bool ResetJumpsOnTooSteepSlopes;
        ...
        [MMReadOnly] [Tooltip("the number of jumps left to the character")]
        ...
        protected bool _buttonReleased;
        ...
        protected float _jumpStartedAt;
        protected bool _jumpStopped;
        ...
        protected override void Initialization()
        {
            ...
            JumpStartFeedback?.Initialization(gameObject);
            JumpStopFeedback?.Initialization(gameObject);
        ...
        protected override void HandleInput()
        {
            ...
            if (!AbilityAuthorized
                ...
                return;
            if (_inputManager.JumpButton.State.CurrentState == MMInput.ButtonStates.ButtonDown) JumpStart();
            if (_inputManager.JumpButton.State.CurrentState == MMInput.ButtonStates.ButtonUp) _buttonReleased = true;
        }
        ...
        {
            if (_controller.JustGotGrounded) ResetNumberOfJumps();
            ...
            if (_movement.CurrentState == CharacterStates.MovementStates.Jumping)
            {
                if (!_jumpStopped)
                {
                    if (transform.position.y - _jumpOrigin.y > JumpHeight
                        || CeilingTest())
                    {
                        JumpStop();
                        _controller3D.Grounded = _controller3D.IsGroundedTest();
                        if (_controller.Grounded) ResetNumberOfJumps();
                    }
                    else
                    {
        ...
        {
            var returnValue = _controller3D.CollidingAbove();
            return returnValue;
        }
        ...
        {
            if (!EvaluateJumpConditions()) return;
            ...
            JumpStartFeedback?.PlayFeedbacks(transform.position);
        ...
        public virtual void JumpStop()
        {
            ...
            if (_controller.Velocity.y > 0) _controller.Velocity.y = 0f;
            _jumpStopped = true;
            _buttonReleased = false;
            PlayAbilityStopSfx();
            StopAbilityUsedSfx();
            ...
            JumpStopFeedback?.PlayFeedbacks(transform.position);
        }
        ...
        {
            var shouldResetJumps = true;

            if (!ResetJumpsOnTooSteepSlopes)
                if (_controller3D.TooSteep())
                    shouldResetJumps = false;

            if (shouldResetJumps) NumberOfJumpsLeft = NumberOfJumps;
        }
        ...
        protected virtual bool EvaluateJumpConditions()
        {
            ...
            if (CeilingTest()) return false;

            if (NumberOfJumpsLeft <= 0) return false;
