...
namespace MoreMountains.TopDownEngine
{
    ...
    public class CharacterRagdollOnDeath : MonoBehaviour
    {
        ...
        {
            foreach (var go in ObjectsToDisableOnDeath) go.SetActive(false);
            foreach (var mono in MonosToDisableOnDeath) mono.enabled = false;
            Ragdoller.Ragdolling = true;
            Ragdoller.transform.SetParent(null);
            Ragdoller.MainRigidbody.AddForce(_controller.AppliedImpact.normalized * ForceMultiplier,
