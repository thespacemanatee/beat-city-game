...
namespace MoreMountains.TopDownEngine
{
    ...
    ///     Note that this will only change the model, not the prefab. Only the visual representation, not the abilities and
    ///     settings.
    ...
    public class CharacterSwitchModel : CharacterAbility
    {
        ...
        [Header("Models")]
        [MMInformation(
            "Add this component to a character and it'll be able to switch its model when pressing the SwitchCharacter button (P by default).",
        ...
        protected override void Initialization()
        {
            ...
            foreach (var model in CharacterModels) model.SetActive(false);
        ...
        {
            if (_characterModelsFlipped == null) _characterModelsFlipped = new bool[CharacterModels.Length];
            if (_characterModelsFlipped.Length == 0) _characterModelsFlipped = new bool[CharacterModels.Length];
            if (_character == null) _character = gameObject.GetComponentInParent<Character>();
        }
        ...
        {
            if (CharacterModels.Length <= 1) return;
            ...
            else
            {
                ...
                if (CurrentIndex >= CharacterModels.Length) CurrentIndex = 0;
            }
            ...
            if (AutoBindAnimator)
            {
                _character.CharacterAnimator =
                    CharacterModels[CurrentIndex].gameObject.MMGetComponentNoAlloc<Animator>();
            ...
            if (_instantiatedVFX != null)
            {
                ...
                _instantiatedVFX.transform.position = transform.position;
