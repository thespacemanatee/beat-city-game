...
namespace MoreMountains.TopDownEngine
{
    ...
    public class CharacterInventory : CharacterAbility, MMEventListener<MMInventoryEvent>
    {
        ...
        protected bool _nextFrameWeapon;
        ...
        ///     When an inventory gets loaded, if it's our WeaponInventory, we check if there's already a weapon equipped, and if
        ///     yes, we equip it
        /// </summary>
        ...
        public virtual void OnMMEvent(MMInventoryEvent inventoryEvent)
        {
            ...
            {
                var isSubclass = inventoryEvent.EventItem.GetType().IsSubclassOf(typeof(InventoryWeapon));
                var isClass = inventoryEvent.EventItem.GetType() == typeof(InventoryWeapon);
                if (isClass || isSubclass)
                {
                    var inventoryWeapon = (InventoryWeapon)inventoryEvent.EventItem;
        ...
        {
            if (InventoryTransform == null) InventoryTransform = transform;
            GrabInventories();
            if (CharacterHandleWeapon == null) CharacterHandleWeapon = _character?.FindAbility<CharacterHandleWeapon>();
            FillAvailableWeaponsLists();

            // we auto pick items if needed
            if (AutoPickItems.Length > 0)
                foreach (var item in AutoPickItems)
        ...
        protected override void HandleInput()
        {
            if (!AbilityAuthorized
                ...
                return;
            if (_inputManager.SwitchWeaponButton.State.CurrentState == MMInput.ButtonStates.ButtonDown) SwitchWeapon();
        }
        ...
        protected virtual void FillAvailableWeaponsLists()
        {
            ...
            if (CharacterHandleWeapon == null || WeaponInventory == null) return;
            _availableWeapons = MainInventory.InventoryContains(ItemClasses.Weapon);
            foreach (var index in _availableWeapons) _availableWeaponsIDs.Add(MainInventory.Content[index].ItemID);
            if (!InventoryItem.IsNull(WeaponInventory.Content[0]))
        ...
        protected virtual void DetermineNextWeaponName()
        {
            ...
            for (var i = 0; i < _availableWeaponsIDs.Count; i++)
        ...
        protected virtual void EquipWeapon(string weaponID)
        {
            if (weaponID == _emptySlotWeaponName && CharacterHandleWeapon != null)
            {
                ...
                CharacterHandleWeapon.ChangeWeapon(null, _emptySlotWeaponName);
            ...
            if (weaponID == _initialSlotWeaponName && CharacterHandleWeapon != null)
            {
                MMInventoryEvent.Trigger(MMInventoryEventType.UnEquipRequest, null, WeaponInventoryName,
                    WeaponInventory.Content[0], 0, 0, PlayerID);
                CharacterHandleWeapon.ChangeWeapon(CharacterHandleWeapon.InitialWeapon, _initialSlotWeaponName);
                MMInventoryEvent.Trigger(MMInventoryEventType.Redraw, null, WeaponInventory.name, null, 0, 0, PlayerID);
                return;
            }

            for (var i = 0; i < MainInventory.Content.Length; i++)
            {
        ...
        protected virtual void SwitchWeapon()
        {
            ...
            if (_availableWeaponsIDs.Count <= 0) return;
