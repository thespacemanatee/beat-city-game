        ...
        public virtual void DashStart()
        {
            ...
            switch (DashMode)
            {
                case DashModes.MainMovement:
                    _dashDestination = transform.position + _controller.CurrentDirection.normalized * DashDistance;
                    break;
                ...
                case DashModes.SecondaryMovement:
                    _dashDestination = transform.position +
                                       (Vector3)_character.LinkedInputManager.SecondaryMovement.normalized *
                                       DashDistance;
                    break;

                case DashModes.MousePosition:
                    _inputPosition = _mainCamera.ScreenToWorldPoint(Input.mousePosition);
                    _inputPosition.z = transform.position.z;
                    _dashDestination = transform.position +
                                       (_inputPosition - transform.position).normalized * DashDistance;
                    break;
            }
        }
