...
namespace MoreMountains.TopDownEngine
{
    ...
    public class WeaponIKDisabler : MonoBehaviour
    {
        ...
        {
            if (_animator == null || _weaponIK == null || WeaponAttachment == null) return;

            if (_animationParametersHashes.Count <= 0) return;

            var shouldPreventIK = false;
            foreach (var hash in _animationParametersHashes)
                if (_animator.GetBool(hash))
                    shouldPreventIK = true;

            if (shouldPreventIK != _shouldSetIKLast) PreventIK(shouldPreventIK);
        ...
        {
            _weaponIK = gameObject.GetComponent<WeaponIK>();
            _animator = gameObject.GetComponent<Animator>();
            _animationParametersHashes = new List<int>();

            foreach (var _animationParameterName in AnimationParametersPreventingIK)
            {
                var newHash = Animator.StringToHash(_animationParameterName);
