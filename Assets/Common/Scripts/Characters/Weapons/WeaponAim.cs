...
namespace MoreMountains.TopDownEngine
{
    ...
    public abstract class WeaponAim : MonoBehaviour, MMEventListener<TopDownEngineEvent>
    {
        ...
        [Header("Control Mode")]
        [MMInformation(
            "Add this component to a Weapon and you'll be able to aim (rotate) it. It supports three different control modes : mouse (the weapon aims towards the pointer), primary movement (you'll aim towards the current input direction), or secondary movement (aims towards a second input axis, think twin stick shooters).",
        ...
        [MMInformation(
            "Here you can define whether the rotation is free, strict in 4 directions (top, bottom, left, right), or 8 directions (same + diagonals). You can also define a rotation speed, and a min and max angle. For example, if you don't want your character to be able to aim in its back, set min angle to -90 and max angle to 90.",
            MMInformationAttribute.InformationType.Info, false)]
        /// the rotation mode
        ...
        [Range(-180, 180)] [Tooltip("the minimum angle at which the weapon's rotation will be clamped")]
        public float MinimumAngle = -180f;

        /// the maximum angle at which the weapon's rotation will be clamped
        [Range(-180, 180)] [Tooltip("the maximum angle at which the weapon's rotation will be clamped")]
        ...
        [Tooltip(
            "whether the camera target should be moved towards the reticle to provide a better vision of the possible target. If you don't have a reticle, it'll be moved towards your aim direction.")]
        public bool MoveCameraTargetTowardsReticle;

        /// the offset to apply to the camera target along the transform / reticle line
        [Range(0f, 1f)] [Tooltip("the offset to apply to the camera target along the transform / reticle line")]
        ...
        public Vector3 CurrentAim => _currentAim;

        /// the weapon's current direction, absolute (flip independent)
        public Vector3 CurrentAimAbsolute => _currentAimAbsolute;
        ...
        {
            if (ReplaceMousePointer) Cursor.visible = true;
        }
        ...
        protected virtual void RotateWeapon(Quaternion newRotation, bool forceInstant = false)
        {
            ...
            // otherwise we lerp the rotation
                ...
                transform.rotation =
                    Quaternion.Slerp(transform.rotation, newRotation, WeaponRotationSpeed * Time.deltaTime);
        }
        ...
        {
            if (AimControl != AimControls.Mouse) return;
            if (GameManager.Instance.Paused)
            {
                Cursor.visible = true;
                return;
        ...
        protected virtual void AutoDetectWeaponMode()
        {
            if (_weapon.Owner.LinkedInputManager != null)
            {
                ...
                if (!_weapon.Owner.LinkedInputManager.ForceWeaponMode && _weapon.Owner.LinkedInputManager.IsMobile &&
                    AimControl == AimControls.Mouse) AimControl = AimControls.PrimaryMovement;
            }
        }
    }
}