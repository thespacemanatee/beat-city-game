...
namespace MoreMountains.TopDownEngine
{
    ...
    public class Projectile : MMPoolableObject
    {
        ...
        [Header("Spawn")]
        [MMInformation(
            "Here you can define an initial delay (in seconds) during which this object won't take or cause damage. This delay starts when the object gets enabled. You can also define whether the projectiles should damage their owner (think rockets and the likes) or not",
        ...
        protected virtual IEnumerator InitialInvulnerability()
        {
            ...
            if (DamageOwner) _damageOnTouch.StopIgnoringObject(_weapon.Owner.gameObject);
        }
        ...
        public virtual void Movement()
        {
            ...
            if (_rigidBody != null) _rigidBody.MovePosition(transform.position + _movement);
            if (_rigidBody2D != null) _rigidBody2D.MovePosition(transform.position + _movement);
        ...
        public virtual void SetDirection(Vector3 newDirection, Quaternion newRotation, bool spawnerIsFacingRight = true)
        {
            ...
            if (ProjectileIsFacingRight != spawnerIsFacingRight) Flip();
            if (FaceDirection) transform.rotation = newRotation;

            if (_damageOnTouch != null) _damageOnTouch.SetScriptDirection(newDirection);
        ...
        {
            _owner = newOwner;
            var damageOnTouch = gameObject.MMGetComponentNoAlloc<DamageOnTouch>();
            if (damageOnTouch != null)
            {
                damageOnTouch.Owner = newOwner;
                damageOnTouch.Owner = newOwner;
                if (!DamageOwner) damageOnTouch.IgnoreGameObject(newOwner);
            }
        }
