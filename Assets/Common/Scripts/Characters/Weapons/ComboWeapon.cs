...
{
    /// <summary>
    ///     Add this component to an object containing multiple weapons and it'll turn it into a ComboWeapon, allowing you to
    ///     chain attacks from all the different weapons
    /// </summary>
    ...
    public class ComboWeapon : MonoBehaviour
    {
        ...
        [MMReadOnly] [Tooltip("the reference to the weapon's Owner")]
        public CharacterHandleWeapon OwnerCharacterHandleWeapon;

        /// the time spent since the last weapon stopped
        [MMReadOnly] [Tooltip("the time spent since the last weapon stopped")]
                ...
                if (_countdownActive && DroppableCombo)
                {
                    ...
                    if (TimeSinceLastWeaponStopped > DropComboDelay)
                    {
                        ...
                        OwnerCharacterHandleWeapon.ChangeWeapon(Weapons[_currentWeaponIndex],
                            Weapons[_currentWeaponIndex].WeaponName, true);
                    }
                }
        }
            ...
            for (var i = 0; i < Weapons.Length; i++)
                if (i != _currentWeaponIndex)
                {
                    Weapons[i].SetOwner(Weapons[_currentWeaponIndex].Owner,
                        Weapons[_currentWeaponIndex].CharacterHandleWeapon);
