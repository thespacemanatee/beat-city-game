...
{
    /// <summary>
    ///     Add this class to a weapon and it'll prevent shooting when close to an obstacle (as defined by the
    ///     ObstacleLayerMask)
    /// </summary>
    ...
    public class WeaponPreventShootingWhenCloseToWalls2D : WeaponPreventShooting
    {
        ...
        {
            _hitLeft = MMDebug.RayCast(transform.position + _weaponAim.CurrentRotation * RaycastOriginOffset,
                (Quaternion.Euler(0f, 0f, -Angle / 2f) * _weaponAim.CurrentAimAbsolute).normalized, Distance,
                ObstacleLayerMask, Color.yellow, true);
            _hitMiddle = MMDebug.RayCast(transform.position + _weaponAim.CurrentRotation * RaycastOriginOffset,
                _weaponAim.CurrentAimAbsolute.normalized, Distance, ObstacleLayerMask, Color.yellow, true);
            _hitRight = MMDebug.RayCast(transform.position + _weaponAim.CurrentRotation * RaycastOriginOffset,
                (Quaternion.Euler(0f, 0f, Angle / 2f) * _weaponAim.CurrentAimAbsolute).normalized, Distance,
                ObstacleLayerMask, Color.yellow, true);
