...
namespace MoreMountains.TopDownEngine
{
    ...
    public class WeaponLaserSight : MonoBehaviour
    {
        ...
        [MMCondition("PerformRaycast")] [Tooltip("if this is false, the laser won't be drawn")]
        ...
        public Vector2 LaserWidth = new(0.05f, 0.05f);
        ...
        protected virtual void Initialization()
        {
            ...
            if (_weapon == null)
                Debug.LogWarning(
                    "This WeaponLaserSight is not associated to a weapon. Please add it to a gameobject with a Weapon component.");
        ...
        public virtual void ShootLaser()
        {
            ...
            _thisForward = transform.forward;
            ...
            if (Mode == Modes.ThreeD)
            {
                ...
                _raycastOrigin =
                    MMMaths.RotatePointAroundPivot(_thisPosition + RaycastOriginOffset, _thisPosition, _thisRotation);

                // we cast a ray in front of the weapon to detect an obstacle
                _hit = MMDebug.Raycast3D(_raycastOrigin, _thisForward, LaserMaxDistance, LaserCollisionMask, Color.red,
                    true);
            ...
            {
                if (_direction == Vector3.left) _laserOffset.x = -LaserOriginOffset.x;

                _raycastOrigin =
                    MMMaths.RotatePointAroundPivot(_weaponPosition + _laserOffset, _weaponPosition, _weaponRotation);
                _origin = _raycastOrigin;
                _direction = _weapon.Flipped ? Vector3.left : Vector3.right;

                // we cast a ray in front of the weapon to detect an obstacle
                _hit2D = MMDebug.RayCast(_raycastOrigin, _weaponRotation * _direction, LaserMaxDistance,
                    LaserCollisionMask, Color.red, true);
                if (_hit2D)
                {
            ...
            if (Time.frameCount <= _initFrame + 1) return;
