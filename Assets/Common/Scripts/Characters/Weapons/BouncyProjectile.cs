...
namespace MoreMountains.TopDownEngine
{
    ...
    public class BouncyProjectile : Projectile
    {
        ...
        {
            if (!BounceLayers.MMContains(collider.layer)) return;

            _raycastDirection = transform.position - _positionLastFrame;

            if (BoundsBasedOn == WaysToDetermineBounds.Collider)
            {
                var hit = MMDebug.Raycast3D(transform.position, Direction.normalized, BounceRaycastLength, BounceLayers,
            ...
            {
                var hit = MMDebug.RayCast(transform.position, Direction.normalized, BounceRaycastLength, BounceLayers,
                    MMColors.DarkOrange, true);
                EvaluateHit2D(hit);
            }
        }
        ...
        protected virtual void Bounce2D(RaycastHit2D hit)
        {
            ...
            var angle = Vector3.Angle(_raycastDirection, _reflectedDirection);
        ...
        protected virtual void Bounce3D(RaycastHit hit)
        {
            ...
            Direction = _reflectedDirection.normalized;
            transform.forward =
                _spawnerIsFacingRight ? _reflectedDirection.normalized : -_reflectedDirection.normalized;
            Speed *= _randomSpeedModifier;
            _bouncesLeft--;
        }
    }
