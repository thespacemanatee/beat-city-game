...
namespace MoreMountains.TopDownEngine
{
    ...
    public class AimMarker : MonoBehaviour
    {
        ...
        {
            FirstTargetFeedback?.Initialization(gameObject);
            NewTargetAssignedFeedback?.Initialization(gameObject);
            NoMoreTargetFeedback?.Initialization(gameObject);
            if (MovementDelay > 0f) _movementDelayWFS = new WaitForSeconds(MovementDelay);
        }
        ...
        public virtual void SetTarget(Transform newTarget)
        {
            ...
            if (_targetLastFrame == null) transform.position = _target.transform.position + Offset;

            if (MovementMode == MovementModes.Instant)
                transform.position = _target.transform.position + Offset;
            else
                MMTween.MoveTransform(this, transform, transform.position, _target.transform.position + Offset,
                    _movementDelayWFS, MovementDelay, MovementDuration, MovementCurve);
        }
        ...
        {
            if (_target == _targetLastFrame) return;
            ...
            if (_targetLastFrame != null && _target != null) NewTargetFound();
        }
