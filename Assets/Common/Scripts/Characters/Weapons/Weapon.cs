...
{
    /// <summary>
    ///     This base class, meant to be extended (see ProjectileWeapon.cs for an example of that) handles rate of fire (rate
    ///     of use actually), and ammo reloading
    /// </summary>
    [SelectionBase]
    public class Weapon : MMMonoBehaviour
    {
        ...
        [MMReadOnly] [Tooltip("whether or not the weapon is currently active")]
        ...
        [Tooltip("the size of the magazine")] public int MagazineSize = 30;
        ...
        [MMReadOnly] [Tooltip("the current amount of ammo loaded inside the weapon")]
        ...
        [MMReadOnly] [Tooltip("if true, the weapon is flipped right now")]
        public bool Flipped;
        ...
        protected bool _controllerIs3D;
        ...
        protected float _reloadingCounter;
        ...
        public virtual void Initialization()
        {
            ...
            _comboWeapon = gameObject.GetComponent<ComboWeapon>();
            _weaponPreventShooting = gameObject.GetComponent<WeaponPreventShooting>();
        ...
        protected virtual void InitializeFeedbacks()
        {
            ...
            WeaponUsedMMFeedbackAlt?.Initialization(gameObject);
            WeaponStopMMFeedback?.Initialization(gameObject);
            WeaponReloadNeededMMFeedback?.Initialization(gameObject);
            WeaponReloadMMFeedback?.Initialization(gameObject);
        }
        ...
        {
            if (_comboWeapon != null) _comboWeapon.Initialization();
        }
            ...
            if (Owner != null)
            {
                ...
                if (CharacterHandleWeapon.AutomaticallyBindAnimator)
                {
                    ...
                    if (_ownerAnimator == null)
                        _ownerAnimator = CharacterHandleWeapon.gameObject.GetComponentInParent<Character>()
                            .CharacterAnimator;
                    if (_ownerAnimator == null)
        ...
        protected virtual void TurnWeaponOn()
        {
            ...
            if (_comboWeapon != null) _comboWeapon.WeaponStarted(this);
            ...
            if (PreventAllAimWhileInUse && _weaponAim != null) _weaponAim.enabled = false;
        }
        ...
        public virtual void TurnWeaponOff()
        {
            ...
            ResetMovementMultiplier();
            if (_comboWeapon != null) _comboWeapon.WeaponStopped(this);
            if (PreventAllMovementWhileInUse && _characterMovement != null)
                _characterMovement.MovementForbidden = false;
            if (PreventAllAimWhileInUse && _weaponAim != null) _weaponAim.enabled = true;
        ...
        public virtual void FlipWeapon()
        {
            ...
            if (Owner.Orientation2D == null) return;
            ...
            if (_comboWeapon != null) _comboWeapon.FlipUnusedWeapons();
        }
        ...
        public virtual IEnumerator WeaponDestruction()
        {
            ...
            if (WeaponID != null)
            {
                ...
                var weaponList = Owner.gameObject.GetComponentInParent<Character>()?.FindAbility<CharacterInventory>()
                    .WeaponInventory.InventoryContains(WeaponID);
                if (weaponList.Count > 0)
                    Owner.gameObject.GetComponentInParent<Character>()?.FindAbility<CharacterInventory>()
        ...
        public virtual void ApplyOffset()
        {
            ...
            {
                if (Flipped) _weaponAttachmentOffset.x = -WeaponAttachmentOffset.x;
        ...
        {
            WeaponStartMMFeedback?.PlayFeedbacks(transform.position);
        }
        ...
        protected virtual void TriggerWeaponUsedFeedback()
        {
            ...
            {
                var random = MMMaths.RollADice(2);
                if (random > 1)
                    WeaponUsedMMFeedbackAlt?.PlayFeedbacks(transform.position);
                else
        ...
        {
            WeaponStopMMFeedback?.PlayFeedbacks(transform.position);
        }
        ...
        {
            WeaponReloadNeededMMFeedback?.PlayFeedbacks(transform.position);
        }
        ...
        {
            WeaponReloadMMFeedback?.PlayFeedbacks(transform.position);
        }
        ...
        public virtual void InitializeAnimatorParameters()
        {
            if (Animators.Count > 0)
                for (var i = 0; i < Animators.Count; i++)
                {
                    ...
                    if (!PerformAnimatorSanityChecks) Animators[i].logWarnings = false;
                }
            ...
            if (_ownerAnimator != null)
            {
                ...
                if (!PerformAnimatorSanityChecks) _ownerAnimator.logWarnings = false;
            }
        }

        protected virtual void AddParametersToAnimator(Animator animator, HashSet<int> list)
        {
            MMAnimatorExtensions.AddAnimatorParameterIfExists(animator, WeaponAngleAnimationParameter,
                out _weaponAngleAnimationParameter, AnimatorControllerParameterType.Float, list);
            MMAnimatorExtensions.AddAnimatorParameterIfExists(animator, WeaponAngleRelativeAnimationParameter,
                out _weaponAngleRelativeAnimationParameter, AnimatorControllerParameterType.Float, list);
            MMAnimatorExtensions.AddAnimatorParameterIfExists(animator, IdleAnimationParameter,
                out _idleAnimationParameter, AnimatorControllerParameterType.Bool, list);
            MMAnimatorExtensions.AddAnimatorParameterIfExists(animator, StartAnimationParameter,
                out _startAnimationParameter, AnimatorControllerParameterType.Bool, list);
            MMAnimatorExtensions.AddAnimatorParameterIfExists(animator, DelayBeforeUseAnimationParameter,
                out _delayBeforeUseAnimationParameter, AnimatorControllerParameterType.Bool, list);
            MMAnimatorExtensions.AddAnimatorParameterIfExists(animator, DelayBetweenUsesAnimationParameter,
                out _delayBetweenUsesAnimationParameter, AnimatorControllerParameterType.Bool, list);
            MMAnimatorExtensions.AddAnimatorParameterIfExists(animator, StopAnimationParameter,
                out _stopAnimationParameter, AnimatorControllerParameterType.Bool, list);
            MMAnimatorExtensions.AddAnimatorParameterIfExists(animator, ReloadStartAnimationParameter,
                out _reloadStartAnimationParameter, AnimatorControllerParameterType.Bool, list);
            MMAnimatorExtensions.AddAnimatorParameterIfExists(animator, ReloadStopAnimationParameter,
                out _reloadStopAnimationParameter, AnimatorControllerParameterType.Bool, list);
            MMAnimatorExtensions.AddAnimatorParameterIfExists(animator, ReloadAnimationParameter,
                out _reloadAnimationParameter, AnimatorControllerParameterType.Bool, list);
            MMAnimatorExtensions.AddAnimatorParameterIfExists(animator, SingleUseAnimationParameter,
                out _singleUseAnimationParameter, AnimatorControllerParameterType.Bool, list);
            MMAnimatorExtensions.AddAnimatorParameterIfExists(animator, UseAnimationParameter,
                out _useAnimationParameter, AnimatorControllerParameterType.Bool, list);

            if (_comboWeapon != null)
                MMAnimatorExtensions.AddAnimatorParameterIfExists(animator,
                    _comboWeapon.ComboInProgressAnimationParameter, out _comboInProgressAnimationParameter,
        ...
        {
            for (var i = 0; i < Animators.Count; i++) UpdateAnimator(Animators[i], _animatorParameters[i]);
        ...
        {
            MMAnimatorExtensions.UpdateAnimatorBool(animator, _idleAnimationParameter,
                WeaponState.CurrentState == WeaponStates.WeaponIdle, list, PerformAnimatorSanityChecks);
            MMAnimatorExtensions.UpdateAnimatorBool(animator, _startAnimationParameter,
                WeaponState.CurrentState == WeaponStates.WeaponStart, list, PerformAnimatorSanityChecks);
            MMAnimatorExtensions.UpdateAnimatorBool(animator, _delayBeforeUseAnimationParameter,
                WeaponState.CurrentState == WeaponStates.WeaponDelayBeforeUse, list, PerformAnimatorSanityChecks);
            MMAnimatorExtensions.UpdateAnimatorBool(animator, _useAnimationParameter,
                ...
                WeaponState.CurrentState == WeaponStates.WeaponUse ||
                WeaponState.CurrentState == WeaponStates.WeaponDelayBetweenUses, list, PerformAnimatorSanityChecks);
            MMAnimatorExtensions.UpdateAnimatorBool(animator, _singleUseAnimationParameter,
                WeaponState.CurrentState == WeaponStates.WeaponUse, list, PerformAnimatorSanityChecks);
            MMAnimatorExtensions.UpdateAnimatorBool(animator, _delayBetweenUsesAnimationParameter,
                WeaponState.CurrentState == WeaponStates.WeaponDelayBetweenUses, list, PerformAnimatorSanityChecks);
            MMAnimatorExtensions.UpdateAnimatorBool(animator, _stopAnimationParameter,
                WeaponState.CurrentState == WeaponStates.WeaponStop, list, PerformAnimatorSanityChecks);
            MMAnimatorExtensions.UpdateAnimatorBool(animator, _reloadStartAnimationParameter,
                WeaponState.CurrentState == WeaponStates.WeaponReloadStart, list, PerformAnimatorSanityChecks);
            MMAnimatorExtensions.UpdateAnimatorBool(animator, _reloadAnimationParameter,
                WeaponState.CurrentState == WeaponStates.WeaponReload, list, PerformAnimatorSanityChecks);
            MMAnimatorExtensions.UpdateAnimatorBool(animator, _reloadStopAnimationParameter,
                WeaponState.CurrentState == WeaponStates.WeaponReloadStop, list, PerformAnimatorSanityChecks);

            if (Owner != null)
                MMAnimatorExtensions.UpdateAnimatorBool(animator, _aliveAnimationParameter,
                    Owner.ConditionState.CurrentState != CharacterStates.CharacterConditions.Dead, list,
                    PerformAnimatorSanityChecks);

            if (_weaponAim != null)
            {
                MMAnimatorExtensions.UpdateAnimatorFloat(animator, _weaponAngleAnimationParameter,
                    _weaponAim.CurrentAngle, list, PerformAnimatorSanityChecks);
                MMAnimatorExtensions.UpdateAnimatorFloat(animator, _weaponAngleRelativeAnimationParameter,
                    _weaponAim.CurrentAngleRelative, list, PerformAnimatorSanityChecks);
            }
            else
            {
            ...
            if (_comboWeapon != null)
                MMAnimatorExtensions.UpdateAnimatorBool(animator, _comboInProgressAnimationParameter,
                    _comboWeapon.ComboInProgress, list, PerformAnimatorSanityChecks);
        }
    }
}