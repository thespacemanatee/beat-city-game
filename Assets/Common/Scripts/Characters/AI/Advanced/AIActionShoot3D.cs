...
{
    /// <summary>
    ///     An Action that shoots using the currently equipped weapon. If your weapon is in auto mode, will shoot until you
    ///     exit the state, and will only shoot once in SemiAuto mode. You can optionnally have the character face (left/right)
    ///     the target, and aim at it (if the weapon has a WeaponAim component).
    /// </summary>
    ...
    public class AIActionShoot3D : AIAction
    {
        ...
        public bool LockVerticalAim;

        protected Character _character;
        protected int _numberOfShoots;
        ...
        protected virtual void Update()
        {
            if (TargetHandleWeaponAbility.CurrentWeapon != null)
                if (_weaponAim != null)
                    if (_shooting)
                    {
                        if (LockVerticalAim) _weaponAimDirection.y = 0;

                        if (AimAtTarget) _weaponAim.SetCurrentAim(_weaponAimDirection);
                    }
        }
                ...
                if (_weaponAim != null)
                {
                    if (_projectileWeapon != null)
                    {
                        ...
                        else if (AimOrigin == AimOrigins.SpawnPosition)
                        {
                            ...
                            _weaponAimDirection =
                                _brain.Target.position + ShootOffset - _projectileWeapon.SpawnPosition;
                        }
                    }
                    else
                    {
        ...
        protected virtual void Shoot()
        {
            ...
            if (_targetWeapon == null || TargetHandleWeaponAbility.CurrentWeapon != _targetWeapon) OnEnterState();
        }
        ...
        public override void OnEnterState()
        {
            ...
            _shooting = true;
            _weaponAim = TargetHandleWeaponAbility.CurrentWeapon.gameObject.MMGetComponentNoAlloc<WeaponAim>();
            _projectileWeapon = TargetHandleWeaponAbility.CurrentWeapon.gameObject
                .MMGetComponentNoAlloc<ProjectileWeapon>();
        }
