...
namespace MoreMountains.TopDownEngine
{
    ...
    ///     Makes the character move randomly in the grid, until it finds an obstacle in its path, in which case it'll pick a
    ...
    public class AIActionMoveRandomlyGrid : AIAction
    {
        ...
        [Header("Dimension")] public Modes Mode = Modes.ThreeD;
        ...
        {
            _characterGridMovement = gameObject.GetComponentInParent<Character>()?.FindAbility<CharacterGridMovement>();
            _topDownController = gameObject.GetComponentInParent<TopDownController>();
        ...
        protected virtual void CheckForObstacles()
        {
            ...
            if (Mode == Modes.ThreeD)
            {
                ...
                _temp3DVector.y = 0;
                _hit = MMDebug.Raycast3D(_collider.bounds.center, _temp3DVector, ObstaclesDetectionDistance,
                    ObstacleLayerMask, Color.gray);
                if (_topDownController.CollidingWithCardinalObstacle) PickNewDirection();
            }
            else
            {
                _temp2DVector = _direction;
                _hit2D = MMDebug.RayCast(_collider2D.bounds.center, _temp2DVector, ObstaclesDetectionDistance,
                    ObstacleLayerMask, Color.gray);
                if (_topDownController.CollidingWithCardinalObstacle) PickNewDirection();
            }
        }
        ...
        protected virtual void PickNewDirection()
        {
            ...
            switch (Mode)
            {
                case Modes.ThreeD:
                    ...
                    {
                        retries++;
                        var random = MMMaths.RollADice(4) - 1;
                        _temp3DVector = _raycastDirections3D[random];

                        if (Avoid180)
                        {
                            if (_temp3DVector.x == -_direction.x && Mathf.Abs(_temp3DVector.x) > 0) continue;
                            if (_temp3DVector.y == -_direction.y && Mathf.Abs(_temp3DVector.y) > 0) continue;
                        }
                ...
                case Modes.TwoD:
                    while (retries < 10)
                    {
                        ...
                        if (Avoid180)
                        {
                            if (_temp2DVector.x == -_direction.x && Mathf.Abs(_temp2DVector.x) > 0) continue;
                            if (_temp2DVector.y == -_direction.y && Mathf.Abs(_temp2DVector.y) > 0) continue;
                        }
