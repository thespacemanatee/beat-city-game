...
namespace MoreMountains.TopDownEngine
{
    ...
    public class AIDecisionDetectTargetLine3D : AIDecision
    {
        ...
        public bool ForceAimToDetectionDirection;
        ...
        {
            if (DetectMethod != DetectMethods.WideRay) return;
            ...
            if (ReferenceTransform != null) Gizmos.matrix = ReferenceTransform.localToWorldMatrix;
        ...
        protected virtual bool DetectTarget()
        {
            ...
            if (DetectMethod == DetectMethods.Ray)
                raycast = MMDebug.Raycast3D(_raycastOrigin, _direction, DetectionDistance, TargetLayer, MMColors.Gold,
                    true);
            else
                hit = Physics.BoxCast(_raycastOrigin, Vector3.one * (RayWidth * 0.5f), _direction, out raycast,
                    ReferenceTransform.rotation, DetectionDistance, TargetLayer);
            ...
            if (hit)
            {
                ...
                var distance = Vector3.Distance(target.transform.position, _raycastOrigin);
                var raycastObstacle = MMDebug.Raycast3D(_raycastOrigin,
                    (target.transform.position - _raycastOrigin).normalized, distance, ObstaclesLayer, Color.gray,
                    true);
        ...
        {
            if (!ForceAimToDetectionDirection) return;
            if (_characterHandleWeapon == null) return;
            if (_characterHandleWeapon.CurrentWeapon == null) return;
            _characterHandleWeapon.CurrentWeapon.gameObject.MMGetComponentNoAlloc<WeaponAim3D>()
                ?.SetCurrentAim(ReferenceTransform.forward);
        }
    }
}