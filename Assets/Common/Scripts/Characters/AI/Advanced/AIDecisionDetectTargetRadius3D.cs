...
{
    /// <summary>
    ///     This decision will return true if an object on its TargetLayer layermask is within its specified radius, false
    ///     otherwise. It will also set the Brain's Target to that object.
    /// </summary>
    ...
    public class AIDecisionDetectTargetRadius3D : AIDecision
    {
        ...
        protected virtual bool DetectTarget()
        {
            ...
            if (Time.time - _lastTargetCheckTimestamp < TargetCheckFrequency) return _lastReturnValue;
            _potentialTargets.Clear();

            _lastTargetCheckTimestamp = Time.time;
            _raycastOrigin = _collider.bounds.center + DetectionOriginOffset / 2;
            var numberOfCollidersFound = Physics.OverlapSphereNonAlloc(_raycastOrigin, Radius, _hits, TargetLayerMask);
            ...
            var min = Mathf.Min(OverlapMaximum, numberOfCollidersFound);
            for (var i = 0; i < min; i++)
            {
                if (_hits[i] == null) continue;

                if (!CanTargetSelf)
                    if (_hits[i].gameObject == _brain.Owner || _hits[i].transform.IsChildOf(transform))
                        continue;
            ...
            foreach (var t in _potentialTargets)
            {
                ...
                var hit = MMDebug.Raycast3D(_raycastOrigin, _raycastDirection, _raycastDirection.magnitude,
