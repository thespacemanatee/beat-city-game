...
{
    /// <summary>
    ///     This decision will return true if the Brain's current target is facing this character. Yes, it's quite specific to
    ///     Ghosts. But hey now you can use it too!
    /// </summary>
    ...
    public class AIDecisionTargetFacingAI2D : AIDecision
    {
        ...
        protected virtual bool EvaluateTargetFacingDirection()
        {
            ...
            {
                if (_orientation2D.IsFacingRight && transform.position.x > _orientation2D.transform.position.x)
                    return true;
                if (!_orientation2D.IsFacingRight && transform.position.x < _orientation2D.transform.position.x)
                    return true;
            }
