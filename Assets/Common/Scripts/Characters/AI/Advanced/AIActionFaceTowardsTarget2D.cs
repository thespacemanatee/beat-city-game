    ...
    public class AIActionFaceTowardsTarget2D : AIAction
    {
        ...
        public override void Initialization()
        {
            _characterOrientation2D =
                gameObject.GetComponentInParent<Character>()?.FindAbility<CharacterOrientation2D>();
        ...
        {
            if (_brain.Target == null || !_chacterOrientation2DNotNull) return;
            _targetPosition = _brain.Target.transform.position;

            if (Mode == Modes.LeftRight)
            {
                var facingDirection = _targetPosition.x < transform.position.x ? -1 : 1;
            ...
            else
            {
                ...
                if (Mathf.Abs(_distance.y) > Mathf.Abs(_distance.x))
                    _newFacingDirection = _distance.y > 0
                        ? Character.FacingDirections.North
                        : Character.FacingDirections.South;
                else
                    _newFacingDirection =
                        _distance.x > 0 ? Character.FacingDirections.East : Character.FacingDirections.West;
