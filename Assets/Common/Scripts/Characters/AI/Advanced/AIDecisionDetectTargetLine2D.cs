...
{
    /// <summary>
    ///     This Decision will return true if any object on its TargetLayer layermask enters its line of sight. It will also
    ///     set the Brain's Target to that object. You can choose to have it in ray mode, in which case its line of sight will
    ///     be an actual line (a raycast), or have it be wider (in which case it'll use a spherecast). You can also specify an
    ///     offset for the ray's origin, and an obstacle layer mask that will block it.
    /// </summary>
    ...
    public class AIDecisionDetectTargetLine2D : AIDecision
    {
        ...
        [Tooltip(
            "the selected detection method : ray is a single ray, wide ray is more expensive but also more accurate")]
        ...
        public Vector3 DetectionOriginOffset = new(0, 0, 0);
        ...
        protected bool _drawLeftGizmo;
        protected bool _drawRightGizmo;
        ...
        protected Vector2 _transformRight => ReferenceTransform.right;
        protected Vector2 _transformLeft => -ReferenceTransform.right;
        protected Vector2 _transformUp => ReferenceTransform.up;
        protected Vector2 _transformDown => -ReferenceTransform.up;
        ...
        {
            if (DetectMethod != DetectMethods.WideRay || !_init) return;
        ...
        protected virtual bool DetectTarget()
        {
                ...
                || (DetectionDirection == DetectionDirections.Back && _isFacingRight))
            {
                ...
                {
                    raycast = MMDebug.RayCast(_raycastOrigin, _transformLeft, DetectionDistance, TargetLayer,
                        MMColors.Gold, true);
                }
                else
                {
                    raycast = Physics2D.BoxCast(_raycastOrigin + Vector2.right * _boxcastSize.x / 2f, _boxcastSize, 0f,
                        _transformLeft, DetectionDistance, TargetLayer);
                    MMDebug.RayCast(_raycastOrigin + _transformUp * RayWidth / 2f, _transformLeft, DetectionDistance,
                        TargetLayer, MMColors.Gold, true);
                    MMDebug.RayCast(_raycastOrigin - _transformUp * RayWidth / 2f, _transformLeft, DetectionDistance,
                        TargetLayer, MMColors.Gold, true);
                    MMDebug.RayCast(_raycastOrigin - _transformUp * RayWidth / 2f + _transformLeft * DetectionDistance,
                        _transformUp, RayWidth, TargetLayer, MMColors.Gold, true);
                    _drawLeftGizmo = true;
                }
                ...
                || (DetectionDirection == DetectionDirections.Back && !_isFacingRight))
            {
                if (DetectMethod == DetectMethods.Ray)
                {
                    raycast = MMDebug.RayCast(_raycastOrigin, _transformRight, DetectionDistance, TargetLayer,
                        MMColors.DarkOrange, true);
                }
                else
                {
                    raycast = Physics2D.BoxCast(_raycastOrigin - _transformRight * _boxcastSize.x / 2f, _boxcastSize,
                        0f, _transformRight, DetectionDistance, TargetLayer);
                    MMDebug.RayCast(_raycastOrigin + _transformUp * RayWidth / 2f, _transformRight, DetectionDistance,
                        TargetLayer, MMColors.DarkOrange, true);
                    MMDebug.RayCast(_raycastOrigin - _transformUp * RayWidth / 2f, _transformRight, DetectionDistance,
                        TargetLayer, MMColors.DarkOrange, true);
                    MMDebug.RayCast(_raycastOrigin - _transformUp * RayWidth / 2f + _transformRight * DetectionDistance,
                        _transformUp, RayWidth, TargetLayer, MMColors.DarkOrange, true);
                    _drawLeftGizmo = true;
                }
            ...
            if (hit)
            {
                ...
                var distance = Vector2.Distance(target.transform.position, _raycastOrigin);
                var raycastObstacle = MMDebug.RayCast(_raycastOrigin,
                    ((Vector2)target.transform.position - _raycastOrigin).normalized, distance, ObstaclesLayer,
                    Color.gray, true);
