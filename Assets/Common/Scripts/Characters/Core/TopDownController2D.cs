...
namespace MoreMountains.TopDownEngine
{
    ...
    public class TopDownController2D : TopDownController
    {
        ...
        [MMReadOnly] [Tooltip("whether or not the character is above a hole right now")]
        ...
        public override bool OnAMovingPlatform => _movingPlatform;
        ...
        public Vector2 ColliderSize
        {
            get
            {
                if (!_boxColliderNull) return _boxCollider.size;
                if (!_capsuleColliderNull) return _capsuleCollider.size;
                if (!_circleColliderNull) return Vector2.one * _circleCollider.radius;
                return Vector2.zero;
            }
            set
            {
                ...
                if (!_circleColliderNull) _circleCollider.radius = value.x;
            }
        }
        ...
        public Vector2 ColliderOffset
        {
            get
            {
                if (!_boxColliderNull) return _boxCollider.offset;
                if (!_capsuleColliderNull) return _capsuleCollider.offset;
                if (!_circleColliderNull) return _circleCollider.offset;
                return Vector2.zero;
            }
            set
            {
                ...
                if (!_circleColliderNull) _circleCollider.offset = value;
            }
        }
        ...
        public Bounds ColliderBounds
        {
            get
            {
                if (!_boxColliderNull) return _boxCollider.bounds;
                if (!_capsuleColliderNull) return _capsuleCollider.bounds;
                if (!_circleColliderNull) return _circleCollider.bounds;
                return new Bounds();
            }
        }
        ...
        protected override void FixedUpdate()
        {
            ...
            if (Friction > 1) CurrentMovement = CurrentMovement / Friction;
            ...
            var newMovement = _rigidBody.position + (Vector2)(CurrentMovement + AddedForce) * Time.fixedDeltaTime;

            if (OnAMovingPlatform) newMovement += (Vector2)_movingPlatform.CurrentSpeed * Time.fixedDeltaTime;
        ...
        protected override void HandleFriction()
        {
            ...
            if (AddedForce.y != 0f) AddForce(AddedForce);
        ...
        {
            if (_impact.magnitude > 0.2f) _rigidBody.AddForce(_impact);
        ...
        {
            var newYOffset = _originalColliderCenter.y - (_originalColliderSize.y - newHeight) / 2;
            var newSize = ColliderSize;
            newSize.y = newHeight;
        ...
        {
            if (CurrentMovement != Vector3.zero) CurrentDirection = CurrentMovement.normalized;
        }
        ...
        {
            if (!_boxColliderNull) _boxCollider.enabled = true;
            if (!_capsuleColliderNull) _capsuleCollider.enabled = true;
            if (!_circleColliderNull) _circleCollider.enabled = true;
        }
        ...
        {
            if (!_boxColliderNull) _boxCollider.enabled = false;
            if (!_capsuleColliderNull) _capsuleCollider.enabled = false;
            if (!_circleColliderNull) _circleCollider.enabled = false;
        }
        ...
        public override void DetectObstacles(float distance, Vector3 offset)
        {
            ...
            _raycastRight = MMDebug.RayCast(transform.position + offset, Vector3.right, distance, ObstaclesLayerMask,
                Color.yellow, true);
            ...
            _raycastLeft = MMDebug.RayCast(transform.position + offset, Vector3.left, distance, ObstaclesLayerMask,
                Color.yellow, true);
            ...
            _raycastUp = MMDebug.RayCast(transform.position + offset, Vector3.up, distance, ObstaclesLayerMask,
                Color.yellow, true);
            if (_raycastUp.collider != null)
            {
            ...
            _raycastDown = MMDebug.RayCast(transform.position + offset, Vector3.down, distance, ObstaclesLayerMask,
                Color.yellow, true);
            if (_raycastDown.collider != null)
            {
