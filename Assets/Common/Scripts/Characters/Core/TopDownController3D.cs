...
namespace MoreMountains.TopDownEngine
{
    ...
    public class TopDownController3D : TopDownController
    {
        ...
        public enum VelocityTransferOnJump
        {
            NoTransfer,
            InitialVelocity,
            FloorVelocity,
        ...
        [MMReadOnly] [Tooltip("the current input sent to this character")]
        ...
        public AnimationCurve SlopeSpeedMultiplier = new(new Keyframe(-90, 1), new Keyframe(0, 1), new Keyframe(90, 0));
        ...
        protected Vector3 _lastHitPoint = new(Mathf.Infinity, 0, 0);
        protected Vector3 _lastHorizontalVelocity;
        protected Matrix4x4 _lastMovingPlatformMatrix;
        protected float _longestDistance = float.MinValue;
        ...
        protected float _smallestDistance = float.MaxValue;
        ...
        protected WaitForFixedUpdate _waitForFixedUpdate = new();

        /// returns the center coordinate of the collider
        public override Vector3 ColliderCenter => transform.position + _characterController.center;

        /// returns the bottom coordinate of the collider
        public override Vector3 ColliderBottom => transform.position + _characterController.center +
                                                  Vector3.down * _characterController.bounds.extents.y;

        /// returns the top coordinate of the collider
        public override Vector3 ColliderTop => transform.position + _characterController.center +
        ...
        public override bool OnAMovingPlatform => ShouldMoveWithPlatformThisFrame();

        /// the speed of the moving platform
        public override Vector3 MovingPlatformSpeed => _movingPlatformVelocity;
        ...
        {
            return Grounded && SlideOnSteepSurfaces && TooSteep();
        }
        ...
        {
            return GroundNormal.y <= Mathf.Cos(_characterController.slopeLimit * Mathf.Deg2Rad);
        }
        ...
        protected virtual void AddInput()
        {
            if (Grounded && TooSteep())
            {
                ...
                _idealVelocity = _idealVelocity + _idealDirection * SlidingSpeedControl +
                                 (InputMoveDirection - _idealDirection) * SlidingDirectionControl;
            ...
            if (Grounded)
            {
                var sideways = Vector3.Cross(Vector3.up, _idealVelocity);
        ...
        protected virtual void MoveWithPlatform()
        {
            ...
            {
                _moveWithPlatformMoveDistance.x =
                    _moveWithPlatformMoveDistance.y = _moveWithPlatformMoveDistance.z = 0f;
                ...
                var yRotation = _moveWithPlatformRotationDiff.eulerAngles.y;
                if (yRotation != 0) _transform.Rotate(0, yRotation, 0);
            }
        }
        ...
        protected virtual void ComputeVelocityDelta()
        {
            ...
            if (Grounded) _motion -= _stickyOffset * Vector3.up;
        }
        ...
        {
            if (CurrentMovement.magnitude > 0f) CurrentDirection = CurrentMovement.normalized;
        }
        ...
        ///     To do so, it casts a ray downwards to get our ground normal, and a ray in the current movement direction to
        ///     (potentially) push rigidbodies
        /// </summary>
        ...
        {
            if (GroundedComputationMode != GroundedComputationModes.Advanced) return;

            _smallestDistance = float.MaxValue;
            _longestDistance = float.MinValue;
            _smallestRaycast = _emptyRaycast;

            // we cast 4 rays downwards to get ground normal
            var offset = _characterController.radius;
            ...
            {
                var adjustedDistance = AdjustDistance(_smallestRaycast.distance);
        ...
        {
            if (!AllowPhysicsInteractions) return;

            // we cast a ray towards our move direction to handle pushing objects
            Physics.Raycast(_transform.position + _characterController.center, CurrentMovement.normalized,
                out _raycastDownHit,
                _characterController.radius + _characterController.skinWidth, ObstaclesLayerMask);

            if (_raycastDownHit.collider != null) HandlePush(_raycastDownHit, _raycastDownHit.point);
        }
        ...
        protected virtual void CastRayDownwards()
        {
            ...
            {
                var adjustedDistance = AdjustDistance(_raycastDownHit.distance);
        ...
        {
            var adjustedDistance = distance - _characterController.height / 2f -
        ...
        protected virtual void OnTriggerEnter(Collider other)
        {
            ...
            if (other.gameObject.MMGetComponentNoAlloc<MovingPlatform3D>() != null)
                ...
                {
                    _onTriggerEnterPushbackDirection = (transform.position - other.transform.position).normalized;
                    Impact(_onTriggerEnterPushbackDirection.normalized,
                        other.gameObject.MMGetComponentNoAlloc<MovingPlatform3D>().PushForce);
                }
        }
        ...
        protected virtual void HandlePush(RaycastHit hit, Vector3 hitPosition)
        {
            ...
            if (_pushedRigidbody == null || _pushedRigidbody.isKinematic) return;
        ...
        protected virtual IEnumerator SubstractNewPlatformVelocity()
        {
                ...
                VelocityTransferMethod == VelocityTransferOnJump.FloorVelocity)
            {
                ...
                {
                    var platform = _movingPlatformCurrentHitCollider;
                    yield return _waitForFixedUpdate;
                    if (Grounded && platform == _movingPlatformCurrentHitCollider) yield return 1;
                }
        ...
        {
            return (Grounded || VelocityTransferMethod == VelocityTransferOnJump.Relative)
        ...
        public override bool CanGoBackToOriginalSize()
        {
            ...
            if (_collider.bounds.size.y == _originalColliderHeight) return true;
            var headCheckDistance = _originalColliderHeight * transform.localScale.y * CrouchedRaycastLengthMultiplier;
            ...
            _canGoBackHeadCheck = MMDebug.Raycast3D(_originalSizeRaycastOrigin, transform.up, headCheckDistance,
        ...
        {
            var newYOffset = _originalColliderCenter.y - (_originalColliderHeight - newHeight) / 2;
        ...
        public virtual bool IsGroundedTest()
        {
            ...
            {
                if (_smallestDistance <= MinimumGroundedDistance) grounded = GroundNormal.y > 0.01;
            }
            else
            {
                grounded = true;
        ...
        {
            if (!PerformCardinalObstacleRaycastDetection) return;

            CollidingWithCardinalObstacle = false;
            // right
            _cardinalRaycast = MMDebug.Raycast3D(transform.position + offset, Vector3.right, distance,
            ...
            // left
            _cardinalRaycast = MMDebug.Raycast3D(transform.position + offset, Vector3.left, distance,
                ObstaclesLayerMask, Color.yellow, true);
            if (_cardinalRaycast.collider != null)
            {
            ...
            // up
            _cardinalRaycast = MMDebug.Raycast3D(transform.position + offset, Vector3.forward, distance,
                ObstaclesLayerMask, Color.yellow, true);
            if (_cardinalRaycast.collider != null)
            {
            ...
            // down
            _cardinalRaycast = MMDebug.Raycast3D(transform.position + offset, Vector3.back, distance,
                ObstaclesLayerMask, Color.yellow, true);
            if (_cardinalRaycast.collider != null)
            {
