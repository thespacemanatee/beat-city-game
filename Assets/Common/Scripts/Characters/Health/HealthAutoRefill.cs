...
{
    /// <summary>
    ///     Add this class to a character or object with a Health class, and its health will auto refill based on the settings
    ///     here
    /// </summary>
    ...
    public class HealthAutoRefill : MonoBehaviour
    {
        ...
        protected virtual void ProcessRefillHealth()
        {
            ...
            if (Time.time - _lastHitTime < CooldownAfterHit) return;
            ...
            if (_health.CurrentHealth < _health.MaximumHealth)
                switch (RefillMode)
                {
                    case RefillModes.Bursts:
                        ...
                        {
                            _health.GetHealth(HealthPerBurst, gameObject);
                    ...
                    case RefillModes.Linear:
                        ...
                        if (_healthToGive > 1f)
                        {
                            var givenHealth = (int)_healthToGive;
                            _healthToGive -= givenHealth;
                            _health.GetHealth(givenHealth, gameObject);
                        }
