...
namespace MoreMountains.Tools
{
    ...
    {
        public enum MMSpriteMaskEventTypes
        {
            MoveToNewPosition,
            ExpandAndMoveToNewPosition,
            DoubleMask
        }
        ...
        public MMSpriteMaskEvent(MMSpriteMaskEventTypes eventType, Vector2 newPosition, Vector2 newSize, float duration,
        ...
        private static MMSpriteMaskEvent e;

        public static void Trigger(MMSpriteMaskEventTypes eventType, Vector2 newPosition, Vector2 newSize,
            float duration, MMTween.MMTweenCurve curve)
        {
    ...
    /// <summary>
    ///     This class will automatically look for sprite renderers, particle systems, tilemaps in the scene, and change their
    ///     SpriteMaskInteraction settings according to the one set in the inspector
    ...
    public class MMSpriteMask : MonoBehaviour, MMEventListener<MMSpriteMaskEvent>
    {
        ...
        public float MaskTime
        {
            get
            {
                var time = Timescale == Timescales.Unscaled ? Time.unscaledTime : Time.time;
                return time;
            }
        }
        ...
        public virtual void OnMMEvent(MMSpriteMaskEvent spriteMaskEvent)
        {
            ...
            switch (spriteMaskEvent.EventType)
            {
                case MMSpriteMaskEvent.MMSpriteMaskEventTypes.MoveToNewPosition:
                    MoveMaskTo(spriteMaskEvent.NewPosition, spriteMaskEvent.NewSize, spriteMaskEvent.Duration,
                        spriteMaskEvent.Curve);
                    break;
                case MMSpriteMaskEvent.MMSpriteMaskEventTypes.ExpandAndMoveToNewPosition:
                    ExpandAndMoveMaskTo(spriteMaskEvent.NewPosition, spriteMaskEvent.NewSize, spriteMaskEvent.Duration,
                        spriteMaskEvent.Curve);
                    break;
            }
        }
        ...
        protected virtual void SetupMaskSettingsAutomatically()
        {
            if (AutomaticallySetupSpriteRenderers)
            {
                ...
                if (foundSpriteRenderers.Length > 0)
                    foreach (var renderer in foundSpriteRenderers)
            ...
            if (AutomaticallySetupTilemaps)
            {
                ...
                if (foundTilemapRenderers.Length > 0)
                    foreach (var renderer in foundTilemapRenderers)
                        if (!renderer.gameObject.CompareTag("NoMask"))
                            renderer.maskInteraction = MaskInteraction;
            }
            ...
            if (AutomaticallySetupParticleSystems)
            {
                ...
                if (foundParticleSystems.Length > 0)
                    foreach (var system in foundParticleSystems)
                        if (!system.gameObject.CompareTag("NoMask"))
                        {
                            var pr = system.GetComponent<ParticleSystemRenderer>();
        ...
        /// <param name="duration"></param>
        /// <param name="curve"></param>
        public virtual void ExpandAndMoveMaskTo(Vector2 newPosition, Vector2 newSize, float duration,
            MMTween.MMTweenCurve curve)
        {
        ...
        /// <param name="curve"></param>
        /// <returns></returns>
        protected virtual IEnumerator MoveMaskToCoroutine(Vector2 newPosition, Vector2 newSize, float duration,
            MMTween.MMTweenCurve curve)
        {
            if (duration > 0)
            {
                ...
                var startedAt = MaskTime;

                while (MaskTime - startedAt <= duration)
                {
                    var currentTime = MaskTime - startedAt;
            ...
            transform.position = ComputeTargetPosition(newPosition);
            transform.localScale = ComputeTargetScale(newSize);
        }

        /// <summary>
        ///     A coroutine that expands the mask to cover both its current position and its destination area, then resizes itself
        ///     to match the destination size
        /// </summary>
        /// <param name="newPosition"></param>
        /// <param name="newSize"></param>
        /// <param name="duration"></param>
        /// <param name="curve"></param>
        /// <returns></returns>
        protected virtual IEnumerator ExpandAndMoveMaskToCoroutine(Vector2 newPosition, Vector2 newSize, float duration,
            MMTween.MMTweenCurve curve)
        {
            if (duration > 0)
            {
                ...
                _targetScale.x = transform.localScale.x / 2f +
                                 Mathf.Abs((transform.position - (Vector3)newPosition).x) * ScaleMultiplier +
                                 ComputeTargetScale(newSize).x / 2f;
                _targetScale.y = transform.localScale.y / 2f +
                                 Mathf.Abs((transform.position - (Vector3)newPosition).y) * ScaleMultiplier +
                                 ComputeTargetScale(newSize).y / 2f;
                _targetScale.z = 1f;

                _targetPosition = (
                    transform.position + Vector3.up * transform.localScale.y / ScaleMultiplier / 2f +
                    Vector3.left * transform.localScale.x / ScaleMultiplier / 2f
                    +
                ...
                while (MaskTime - startedAt <= duration / 2f)
                {
                    ...
                    _newPosition = MMTween.Tween(currentTime, 0f, duration / 2f, _initialPosition, _targetPosition,
                        curve);
