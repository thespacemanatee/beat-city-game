...
namespace MoreMountains.TopDownEngine
{
    ...
    public class GridManager : MMSingleton<GridManager>
    {
        ...
        [MMCondition("DrawDebugGrid", true)] [Tooltip("the mode in which to draw the debug grid")]
        public DebugDrawModes DebugDrawMode = DebugDrawModes.TwoD;

        /// the size (in squares of the debug grid)
        [MMCondition("DrawDebugGrid", true)] [Tooltip("the size (in squares of the debug grid)")]
        public int DebugGridSize = 30;

        /// the color to use to draw the debug grid lines
        [MMCondition("DrawDebugGrid", true)] [Tooltip("the color to use to draw the debug grid lines")]
        public Color CellBorderColor = new(60f, 221f, 255f, 1f);

        /// the color to use to draw the debug grid cells backgrounds
        [MMCondition("DrawDebugGrid", true)] [Tooltip("the color to use to draw the debug grid cells backgrounds")]
        public Color InnerColor = new(60f, 221f, 255f, 0.3f);

        /// a list of all cells currently occupied
        [HideInInspector] public List<Vector3> OccupiedGridCells;
        ...
        [HideInInspector] public Dictionary<GameObject, Vector3Int> LastPositions;

        /// a dictionary holding all next positions registered by objects traveling on the grid
        [HideInInspector] public Dictionary<GameObject, Vector3Int> NextPositions;
        ...
        protected virtual void OnDrawGizmos()
        {
            ...
            if (DebugDrawMode == DebugDrawModes.ThreeD)
            {
                ...
                for (var a = -DebugGridSize; a < DebugGridSize; a++)
                for (var b = -DebugGridSize; b < DebugGridSize; b++)
                {
                    if (a % 2 == 0 && b % 2 != 0) DrawCell3D(a, b);
                    if (a % 2 != 0 && b % 2 == 0) DrawCell3D(a, b);
                }
            }
            else
        ...
        {
            if (!OccupiedGridCells.Contains(cellCoordinates)) OccupiedGridCells.Add(cellCoordinates);
        }
        ...
        {
            if (OccupiedGridCells.Contains(cellCoordinates)) OccupiedGridCells.Remove(cellCoordinates);
        }
        ...
        /// <returns></returns>
        [Obsolete(
            "As of v1.8 of the TopDown Engine, that method is obsolete, you should use WorldToCellCoordinates instead")]
