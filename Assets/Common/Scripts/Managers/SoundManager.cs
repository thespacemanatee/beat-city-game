...
namespace MoreMountains.TopDownEngine
{
    ...
    /// </summary>
    [Obsolete(
        "This SoundManager is now obsolete, and has been replaced by the bigger, better, faster MMSoundManager. It will be removed definitely in an upcoming update. You should remove this one from this scene, and add a MMSoundManager in its place.")]
    public class SoundManager : MMPersistentSingleton<SoundManager>, MMEventListener<TopDownEngineEvent>,
        MMEventListener<MMGameEvent>
    {
        ...
        protected List<AudioSource> _loopingSounds = new();

        public bool IsMusicOn
        {
            get => Settings.MusicOn;
            internal set => Settings.MusicOn = value;
        }

        public bool IsSfxOn
        {
            get => Settings.SfxOn;
            internal set => Settings.SfxOn = value;
        }
        ...
        public virtual void OnMMEvent(MMGameEvent gameEvent)
        {
            if (MuteSfxOnPause)
            {
                if (gameEvent.EventName == "inventoryOpens") MuteAllSfx();
                if (gameEvent.EventName == "inventoryCloses") UnmuteAllSfx();
            }
        }
        ...
        /// <returns></returns>
        public virtual AudioSource PlaySound(AudioClip sfx, Vector3 location, float pitch, float pan,
            float spatialBlend = 0.0f, float volumeMultiplier = 1.0f, bool loop = false,
            AudioSource reuseSource = null, AudioMixerGroup audioGroup = null)
        {
            if (!Settings.SfxOn || !sfx) return null;
            ...
            if (audioSource == null)
            {
                ...
                // we add an audio source to that host
                var newAudioSource = temporaryAudioHost.AddComponent<AudioSource>();
            ...
            if (loop) _loopingSounds.Add(audioSource);
        ...
        {
            var settings = (SoundSettings)MMSaveLoadManager.Load(typeof(SoundSettings), _saveFileName, _saveFolderName);
        ...
        {
            foreach (var source in _loopingSounds)
                if (source != null)
                    source.Stop();
        }
        ...
        {
            if (_backgroundMusic != null) _backgroundMusic.mute = false;
        }
        ...
        {
            if (_backgroundMusic != null) _backgroundMusic.mute = true;
        }
        ...
        public virtual void OnMMSfxEvent(AudioClip clipToPlay, AudioMixerGroup audioGroup = null, float volume = 1f,
            float pitch = 1f)
        {
            PlaySound(clipToPlay, transform.position, pitch, 0.0f, 0.0f, volume, false, audioGroup: audioGroup);
        }
    }
}