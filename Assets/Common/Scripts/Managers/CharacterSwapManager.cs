...
{
    /// <summary>
    ///     Add this class to an empty component in your scene, and it'll allow you to swap characters in your scene when
    ///     pressing the SwapButton (P, by default)
    ...
    public class CharacterSwapManager : MMSingleton<CharacterSwapManager>
    {
        ...
        protected TopDownEngineEvent _swapEvent = new(TopDownEngineEventTypes.CharacterSwap, null);
        ...
        /// <param name="b"></param>
        /// <returns></returns>
        private static int SortSwapsByOrder(CharacterSwap a, CharacterSwap b)
        {
        ...
        {
            if (Input.GetButtonDown(SwapButtonName)) SwapCharacter();
        }
        ...
        {
            if (_characterSwapList.Count < 2) return;

            var newIndex = -1;

            for (var i = 0; i < _characterSwapList.Count; i++)
            ...
            if (newIndex >= _characterSwapList.Count) newIndex = 0;
            _characterSwapList[newIndex].SwapToThisCharacter();

            LevelManager.Instance.Players[0] =
                _characterSwapList[newIndex].gameObject.GetComponentInParent<Character>();
