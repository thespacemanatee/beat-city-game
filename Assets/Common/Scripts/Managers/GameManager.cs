...
namespace MoreMountains.TopDownEngine
{
        ...
        MMEventListener<TopDownEnginePointEvent>
    {
        ...
        public virtual void OnMMEvent(MMGameEvent gameEvent)
        {
            ...
            {
                case "inventoryOpens":
                    if (PauseGameWhenInventoryOpens) Pause(PauseMethods.NoPauseMenu);
                    break;
        ...
        {
            MaximumLives += lives;
            if (increaseCurrent) CurrentLives += lives;
        }
        ...
        protected virtual void SetActiveInventoryInputManager(bool status)
        {
            ...
            if (_inventoryInputManager != null) _inventoryInputManager.enabled = status;
        }
        ...
        public virtual void Pause(PauseMethods pauseMethod = PauseMethods.PauseMenu)
        {
            ...
            if (Time.timeScale > 0.0f)
            {
                ...
                if (pauseMethod == PauseMethods.NoPauseMenu) _inventoryOpen = true;
            }
            else
            {
                UnPause(pauseMethod);
            ...
            Character.FacingDirections facingDirection)
        {
            if (PointsOfEntry.Count > 0)
                foreach (var point in PointsOfEntry)
