...
{
    /// <summary>
    ///     Add this component to an empty object in your scene, and when you'll press the SwitchCharacter button (P by
    ///     default, change that in Unity's InputManager settings),
    ///     your main character will be replaced by one of the prefabs in the list set on this component. You can decide the
    ///     order (sequential or random), and have as many as you want.
    ...
    ///     If you want to swap characters between a bunch of characters within a scene, look at the CharacterSwap ability and
    ...
    public class CharacterSwitchManager : MonoBehaviour
    {
        ...
        protected TopDownEngineEvent _switchEvent = new(TopDownEngineEventTypes.CharacterSwitch, null);
        ...
        {
            if (_instantiatedCharacters.Length <= 1) return;
            ...
            else
            {
                ...
                if (CurrentIndex >= _instantiatedCharacters.Length) CurrentIndex = 0;
            }
            ...
            _instantiatedCharacters[CurrentIndex].transform.position =
                LevelManager.Instance.Players[0].transform.position;
            _instantiatedCharacters[CurrentIndex].transform.rotation =
                LevelManager.Instance.Players[0].transform.rotation;

            // we keep the health if needed
            if (CommonHealth)
                _instantiatedCharacters[CurrentIndex].gameObject.MMGetComponentNoAlloc<Health>().SetHealth(LevelManager
                    .Instance.Players[0].gameObject.MMGetComponentNoAlloc<Health>().CurrentHealth);

            // we put it in the same state the old one was in
            _instantiatedCharacters[CurrentIndex].MovementState
                .ChangeState(LevelManager.Instance.Players[0].MovementState.CurrentState);
            _instantiatedCharacters[CurrentIndex].ConditionState
                .ChangeState(LevelManager.Instance.Players[0].ConditionState.CurrentState);
