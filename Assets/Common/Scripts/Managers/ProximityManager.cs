...
{
    /// <summary>
    ///     A class that looks for objects with a ProximityManaged class on them, and enables/disables them based on their
    ///     settings.
    ///     This class is meant as an example of how you can deal with large scenes with a lot of objects, disabling the ones
    ///     that are far
    ///     away from the action to save on performance.
    ///     Note that there are many ways to do it, this one is simple and generic, there may be better choices for your
    ///     specific use case.
    /// </summary>
    public class ProximityManager : MonoBehaviour, MMEventListener<TopDownEngineEvent>
    {
        ...
        protected float _lastEvaluationAt;
        ...
        protected virtual void GrabControlledObjects()
        {
            if (AutomaticallyGrabControlledObjects)
            {
                ...
                foreach (var managed in items) ControlledObjects.Add(managed);
            }
        }
        ...
        {
            if (AutomaticallySetPlayerAsTarget) ProximityTarget = LevelManager.Instance.Players[0].transform;
        }
        ...
        protected virtual void EvaluateDistance()
        {
            ...
            if (Time.time - _lastEvaluationAt > EvaluationFrequency)
                ...
                return;
            foreach (var proxy in ControlledObjects)
            {
                var distance = Vector3.Distance(proxy.transform.position, ProximityTarget.position);
