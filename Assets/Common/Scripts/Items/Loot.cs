...
namespace MoreMountains.TopDownEngine
{
    ...
    public class Loot : MonoBehaviour
    {
        ...
        public static List<MMSimpleObjectPooler> SimplePoolers = new();
        public static List<MMMultipleObjectPooler> MultiplePoolers = new();
        ...
        [Tooltip(
            "the selected loot mode : - unique : a simple object  - loot table : a LootTable specific to this Loot object - loot definition : a LootTable scriptable object (created by right click > Create > MoreMountains > TopDown Engine > Loot Definition. This loot definition can then be reused in other Loot objects.")]
        public LootModes LootMode = LootModes.Unique;

        /// the object to loot, when in LootMode
        [Tooltip("the object to loot, when in LootMode")] [MMEnumCondition("LootMode", (int)LootModes.Unique)]
        public GameObject GameObjectToLoot;

        /// a loot table defining what objects to spawn
        [Tooltip("a loot table defining what objects to spawn")] [MMEnumCondition("LootMode", (int)LootModes.LootTable)]
        ...
        [Tooltip("the minimum and maximum quantity of objects to spawn")] [MMVector("Min", "Max")]
        ...
        [Tooltip("whether collision detection should happen in 2D or 3D")] [MMCondition("AvoidObstacles", true)]
        ...
        [Tooltip("a debug button used to trigger a loot")] [MMInspectorButton("SpawnLootDebug")]
        ...
        {
            if (DrawGizmos)
                MMSpawnAround.DrawGizmos(SpawnProperties, transform.position, GizmosQuantity, GimosSize, GizmosColor);
        }
        ...
        public virtual void InitializeLootTable()
        {
            switch (LootMode)
            {
                case LootModes.LootTableScriptableObject:
                    if (LootTableSO != null) LootTableSO.ComputeWeights();
                    break;
        ...
        {
            foreach (var simplePooler in SimplePoolers)
                if (simplePooler.GameObjectToPool == GameObjectToLoot)
                    return simplePooler;
            // if we haven't found one, we create one
            var newObject = new GameObject("[MMSimpleObjectPooler] " + GameObjectToLoot.name);
            var pooler = newObject.AddComponent<MMSimpleObjectPooler>();
        ...
        {
            foreach (var multiplePooler in MultiplePoolers)
                if (multiplePooler != null && multiplePooler.MutualizedPoolName == MutualizedPoolName &&
                    multiplePooler.gameObject.scene == gameObject.scene)
                    return multiplePooler;
            // if we haven't found one, we create one
            var newObject = new GameObject("[MMMultipleObjectPooler] " + MutualizedPoolName);
            var pooler = newObject.AddComponent<MMMultipleObjectPooler>();
            ...
            if (LootMode == LootModes.LootTable)
                foreach (var loot in LootTable.ObjectsToLoot)
                {
                    var objectToPool = new MMMultipleObjectPoolerObject();
                    objectToPool.PoolSize = PoolSize;
                    objectToPool.GameObjectToPool = loot.Loot;
                    pooler.Pool.Add(objectToPool);
                }
            else if (LootMode == LootModes.LootTableScriptableObject)
                foreach (var loot in LootTableSO.LootTable.ObjectsToLoot)
                {
                    var objectToPool = new MMMultipleObjectPoolerObject
                    {
                        PoolSize = PoolSize,
        ...
        protected virtual IEnumerator SpawnLootCo()
        {
            ...
            var randomQuantity = Random.Range((int)Quantity.x, (int)Quantity.y);
        ...
        public virtual void SpawnOneLoot()
        {
            ...
            if (AvoidObstacles)
            {
                ...
                {
                    MMSpawnAround.ApplySpawnAroundProperties(_spawnedObject, SpawnProperties, transform.position);

                    if (DimensionMode == DimensionModes.TwoD)
                    {
                        _raycastOrigin = _spawnedObject.transform.position;
                        _raycastHit2D = Physics2D.BoxCast(_raycastOrigin + Vector3.right * AvoidRadius,
                            AvoidRadius * Vector2.one, 0f, Vector2.left, AvoidRadius, AvoidObstaclesLayerMask);
                    ...
                    {
                        _raycastOrigin = _spawnedObject.transform.position;
                        _overlapBox = Physics.OverlapBox(_raycastOrigin, Vector3.one * AvoidRadius, Quaternion.identity,
            ...
            if (_spawnedObject != null) _spawnedObject.gameObject.SetActive(true);
