...
namespace MoreMountains.TopDownEngine
{
    ...
    ///     To give it a try, in the KoalaDungeon demo scene for example, create a new empty game object, add a box collider 2D
    ///     to it, make sure it's trigger.
    ///     then add this component to it. Select the Koala prefab, uncheck "permit ability" on its Dash 2D ability. Now play
    ///     the scene,
    ...
    public class PickableDash2D : PickableItem
    {
        ...
        protected override bool CheckIfPickable()
        {
            ...
            if (_character.CharacterType != Character.CharacterTypes.Player) return false;
            return true;
        }
        ...
        {
            _character.gameObject.MMGetComponentNoAlloc<Character>()?.FindAbility<CharacterDash2D>()
                ?.PermitAbility(true);
        }
    }
}